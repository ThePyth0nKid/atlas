...
**Phase 8.5 SHIPPED 2026-05-13:** Phase-8-consolidation commit + bulletproof handoff for the next session (PR #84).

**Phase 9 SHIPPED 2026-05-13:** W17a ArcadeDB driver scaffold + ADR-Atlas-011 (PR #85). Production `GraphStateBackend` trait + `WorkspaceTxn` trait in `crates/atlas-projector/src/backend/mod.rs` (~555 LOC). `InMemoryBackend` impl (~514 LOC) wraps the existing V2-α `GraphState` pipeline via `Arc<Mutex<HashMap<WorkspaceId, GraphState>>>`; byte-determinism preserved — pinned hex `8962c1681a44f9569f78c5917f568c5a027ac69f727f23ba5e8f871e5e013ac4` UNCHANGED through both the legacy test path AND the new `byte_pin_through_in_memory_backend` trait-conformance test. `ArcadeDbBackend` stub (~203 LOC) compiles with all trait methods `unimplemented!()` + no `reqwest` dep yet (W17b adds it alongside first method body). 8 new trait-conformance tests. ADR-Atlas-011 closes ADR-Atlas-010 OQ-1 (`Box<dyn WorkspaceTxn>` chosen — object safety over associated type) + OQ-2 (`batch_upsert` with vertices-before-edges ordering). Parallel external reviewer dispatch: code-reviewer + security-reviewer both APPROVE, 0 CRITICAL / 0 HIGH, 5 MEDIUM (1 applied in-commit, 4 documented as W17b/V2-γ carry-overs in the plan-doc). All 7 V2-α byte-determinism CI pins byte-identical.

**Phase 9.5 SHIPPED 2026-05-13:** Phase-9-consolidation + W17b ready-to-dispatch prompt skeleton (PR #86).

**Phase 10-counsel SHIPPED 2026-05-14:** RFP-ready counsel-engagement scope-doc (`.handoff/v2-counsel-engagement-scope.md`, 269 lines) + README Art. 12 verbatim fix + pin-file sync (PR #87). Operational unblock for `DECISION-COUNSEL-1`: Nelson selects lead firm and signs engagement letter to start the 6-8-week clock for the GDPR Art. 4(1) hash-as-PII Path-B opinion.

**Phase 10-cleanup SHIPPED 2026-05-14:** W17a trait-surface hardening before W17b first method body lands SemVer-breaking (PR #88). `begin()` lifetime `'_` → `'static`; `pub fn check_workspace_id` + `pub fn check_value_depth_and_size` boundary helpers; `ProjectorError::InvalidWorkspaceId` variant. Resolves 3 of 4 W17a carry-over MEDIUMs at the trait surface; #5 V2-γ-deferred. ADR-Atlas-011 §4.3 amendment.

**Phase 10 SHIPPED 2026-05-14:** **W17b ArcadeDB driver implementation (PR #90)** — production `reqwest::blocking`-based HTTP driver against ArcadeDB Server-mode per ADR-Atlas-010 §4. Sub-module split `crates/atlas-projector/src/backend/arcadedb/{mod.rs, client.rs, cypher.rs}` (~1860 LOC); `reqwest 0.12 + rustls-tls + blocking` dep added; cross-backend byte-determinism test `#[ignore]`-gated behind `ATLAS_ARCADEDB_URL` (live-run wires in W17c). All 4 W17a carry-over MEDIUMs closed (#5 V2-γ-deferred unchanged). Parallel `code-reviewer` + `security-reviewer` dispatch (Lesson #8) found 0 CRITICAL + 2 HIGH + 3 MEDIUM + 2 LOW; all fixed in-commit before merge (incl. `format!("create database {db_name}")` admin-command injection surface closed via stricter db-name `[a-zA-Z0-9_]` allowlist, `run_command` Value-return narrowed to `()` to close ADR-011 §4.3 #12 latent bypass, `SecretString` visibility tightened to `pub(crate)`, scheme + userinfo guard in `ArcadeDbBackend::new()`). 153 tests green; byte-pin `8962c1681a44f9569f78c5917f568c5a027ac69f727f23ba5e8f871e5e013ac4` reproduced through the trait surface; `cargo clippy -p atlas-projector --no-deps -- -D warnings` zero warnings.

**Phase 12 SHIPPED 2026-05-15:** **W18 Phase A — Mem0g Layer-3 cache design (PR #95, `3f228be`).** Spike (`docs/V2-BETA-MEM0G-SPIKE.md`, ~520 lines, 13 sections) + ADR-Atlas-012 (~430 lines, 9 sections, **8 binding sub-decisions**) + plan-doc (~280 lines). LOCKED implementation: `lancedb 0.29.0` + `fastembed-rs 5.13.4` paired (both Apache-2.0, both pure-Rust embedded), NEW workspace member crate `crates/atlas-mem0g/`. Mem0-Python rejected on three blockers (Hermes-skill distribution + cloud-default embedder + delegated secure-delete); Qdrant sidecar reserved as documented pivot (LP1-LP5 trigger thresholds). Critical clarification: *"Mem0g"* is a research-paper name (arXiv:2504.19413), not a separate product. New event-kind `embedding_erased` with EU-DPA-evidentiary payload (`event_id` + `workspace_id` + `erased_at` + optional `requestor_did` + `reason_code`); secure-delete pre-capture-then-lock-then-overwrite protocol (covers fragments + HNSW `_indices/`); cache-invalidation hybrid Layer-1-native triple (TTL + erasure-event + Layer-1 head-divergence); timing-side-channel mitigation via response-time normalisation. Architectural posture preserved: embeddings live OUTSIDE canonicalisation pipeline (lib.rs invariant #3 honoured); byte-pin `8962c168...e013ac4` reproduces; Layer 3 NEVER trust-authoritative — every `SemanticHit` cites back to Layer-1 `event_uuid`. Reviewer dispatch (parallel `code-reviewer` + `security-reviewer`, Lesson #8): 0 CRITICAL + 2 HIGH + 10 MEDIUM + 4 LOW; all HIGH + MEDIUM applied in-commit (notable: secure-delete TOCTOU race rewritten as pre-capture-then-lock; model-download URL pinning bound to three Atlas-source-pinned consts).

**Phase 11 SHIPPED 2026-05-14:** **W17c ArcadeDB Docker-Compose CI + benchmark + W17b Cypher hotfix (PR #92, `61ef036`).** New `.github/workflows/atlas-arcadedb-smoke.yml` Linux lane + `infra/docker-compose.arcadedb-smoke.yml` (ArcadeDB 24.10.1 sidecar) + `tests/arcadedb_benchmark.rs` (B1 sanity, B2 incremental-upsert p50/p95/p99, B3 sorted-read p50/p95/p99 over 50v/100e fixture). The cross-backend byte-determinism test reproduced the byte-pin `8962c168...e013ac4` through the live ArcadeDB driver — first time end-to-end since W17b shipped. **Two W17b regressions surfaced and fixed atomically in PR #92:** (1) reserved Cypher param names (`$from` / `$to` / `$label` silently empty result sets / raise IllegalArgumentException due to SQL+TinkerPop keyword collisions) renamed to `$src` / `$dst` / `$lbl`; (2) Edge type not auto-registered by MERGE — new `ensure_schema_types_exist` runs a single atomic `CREATE ... WITH ... DETACH DELETE` Cypher statement on first `begin()` per workspace (idempotent, per-instance HashSet cache). Reviewer dispatch (Lesson #8): 0 CRITICAL + 1 HIGH (atomic schema-bootstrap to close orphan-sentinel window) + 4 MEDIUM (unauthenticated healthcheck so credentials never appear in `docker inspect`; `set +x` guard against `RUNNER_DEBUG`; `dtolnay/rust-toolchain` branch-tip SHA documented; Mutex TOCTOU doc accuracy) + 2 LOW, all fixed in-commit. T2 trigger (p99 > 15 ms for Read-API depth-3 at 10M-vertex scale) remains a deployment-telemetry observation per ADR-010 §4.4; CI captures the shoulder baseline.

**Phase 13 SHIPPED 2026-05-14:** **W18b Mem0g Layer-3 cache implementation (PR #97, `2f2238b`).** NEW workspace member crate `crates/atlas-mem0g/` (~2300 LOC across `Cargo.toml` + `src/{lib,embedder,secure_delete,lancedb_backend}.rs` + `tests/{embedding_determinism,secure_delete_correctness,mem0g_benchmark}.rs`) implementing ADR-Atlas-012 §4 sub-decisions 1-8 as a W17a-pattern Phase-A-scaffold: production-shape `SemanticCacheBackend` trait + 7-step pre-capture-then-lock-then-overwrite protocol + Atlas-controlled supply-chain verification path + `embedding_erased` Layer-1 dispatch arm + `embedding_erasures` GraphState extension + B4/B5/B6 benches + `atlas-mem0g-smoke` CI + `semantic-search` Read-API with timing-side-channel mitigation. **Scaffold posture:** LanceDB ANN/search body fill-in deferred to W18c pending spike §12 V1-V4 verification (LanceDB Windows behaviour + fastembed-rs cross-platform determinism + Lance v2.2 `_deletion_files` semantics + model size measurement); supply-chain pins (`HF_REVISION_SHA` + `ONNX_SHA256` + `MODEL_URL`) ship as `TODO_W18B_NELSON_VERIFY_*` placeholders with fail-closed posture (`AtlasEmbedder::new` returns `Mem0gError::Embedder("supply-chain gate: ...")` unconditionally until Nelson lifts pre-V2-β-1-ship). Parallel reviewer dispatch (Atlas Standing Protocol Lesson #8): **0 CRITICAL + 4 unique HIGH + 6 MEDIUM**. All HIGH + MEDIUM applied in-commit per Lesson #3 (fix-commit `717922c` on top of initial `80f6957`). Notable closes: (H-1) real SHA-256 via `sha2::Sha256` replacing blake3-prefixed placeholder; (H-2) `AtlasEmbedder::new` unconditional fail-closed bypassing previous `Default::default()` supply-chain bypass (fastembed `try_new_from_user_defined` wiring is W18c work); (H-3) gatekeeper test asserts all three constants; (H-4) `getrandom` OS CSPRNG replacing deterministic blake3-keyed overwrite. Posture preserved: embeddings OUTSIDE canonicalisation pipeline (lib.rs invariant #3 honoured); byte-pin `8962c1681a44f9569f78c5917f568c5a027ac69f727f23ba5e8f871e5e013ac4` reproduces; Layer 3 NEVER trust-authoritative — every `SemanticHit` cites back to Layer-1 `event_uuid`. 578 tests pass workspace-wide; clippy `-D warnings` zero warnings; all 7 CI checks green (incl. NEW `atlas-mem0g-smoke` workflow).

**Phase 13.5 SHIPPED 2026-05-15:** Phase-13 consolidation commit — CHANGELOG `[Unreleased]` W18b entry + master-plan §6 status flip + decisions.md `DECISION-ARCH-W18b` entry + orchestration-plan + dependency-graph + handoff §0z5 W18b SHIPPED narrative + §0-NEXT refresh to W19 ship convergence (with W18c parallel-track note for LanceDB body fill-in + Nelson supply-chain constant lift).

**Phase 13.6 SHIPPED 2026-05-15:** **W19 + W18c plan-docs master-resident handoff-prep (PR #99, `11dcae8`).** NEW `.handoff/v2-beta-welle-19-plan.md` (~280 lines, ship convergence plan with 16-item acceptance criteria + 6-risk register + subagent dispatch prompt skeleton) + NEW `.handoff/v2-beta-welle-18c-plan.md` (~200 lines, 4-phase parallel-track activation plan with 7-risk register). Strategic rationale: replicate the W18 plan-doc pattern (master-resident plan saves dispatch cognitive load) for both upcoming work-streams. No code touched; CI risk LOW.

**Phase 14 W18c Phase A SHIPPED 2026-05-15:** **W18c Phase A — Mem0g supply-chain constants lifted (PR #100, `28700ae`).** 9 compile-in supply-chain pins lifted in `crates/atlas-mem0g/src/embedder.rs` (3 W18b TODO placeholders + 6 NEW Phase-B-prep constants — 1 × SHA-1 revision + 4 × SHA-256 file digests + 4 × URLs) against HuggingFace `BAAI/bge-small-en-v1.5` at revision `5c38ec7c405ec4b44b94cc5a9bb96e735b38267a`, resolved via NEW `tools/w18c-phase-a-resolve.sh` auditable helper (re-runnable for future revision rotations). ONNX file size 133,093,490 bytes / 126.93 MB matches spike §3.4 expected envelope — closes V4 verification gap. Test surface: W18b `pins_are_placeholder_until_nelson_verifies` gatekeeper RETIRED (purpose served); `pins_well_formed_after_lift` UPGRADED to unconditional structural enforcement across all 9 pins (length + lowercase-hex + huggingface.co origin + revision-SHA-in-path invariants). `AtlasEmbedder::new` body UNCHANGED at runtime — still fail-closed pending W18c Phase B `try_new_from_user_defined` wiring; error message + docstrings refreshed. Parallel reviewer dispatch (Lesson #8): **0 CRITICAL + 0 HIGH + 1 overlapping MEDIUM** (`python` → `python3` portability) + **3-4 overlapping LOWs** (SHA-count off-by-one, TMPDIR shadowing, missing sha256-output validation, stale section header), all applied in-commit per Lesson #3 (fix-commit `a66728e` on top of initial `5946a1b`). All 3 triggered CI checks GREEN (`Verify trust-root` 6 s + `atlas-web-playwright` 4m09s + `mem0g-smoke` 1m11s — first live-infra Layer-3 validation post-Phase-A).

**Phase 14.5 SHIPPED 2026-05-15:** Phase-14 W18c-A consolidation commit (PR #101 `31e3756`) — CHANGELOG `[Unreleased]` Phase-14.5 + W18c-A entries + master-plan §6 status flip (Phase 13.6 SHIPPED + Phase 14 W18c-A SHIPPED + Phase 14.5 SHIPPED rows; "Phase 14 next" → "Phase 14 partially complete") + decisions.md `DECISION-ARCH-W18c-A` entry (28th decision) + handoff §0z6 W18c-A SHIPPED narrative + §0-NEXT refresh (W19 ship OR W18c Phase B as primary/alternate paths; Nelson-blocked items list reflects W18c Phase A complete).

**Phase 14 W19 SHIPPED 2026-05-15:** **W19 v2.0.0-beta.1 ship convergence milestone (PR #102 `1f5a51f` ship + PR #103 `81d363e` post-merge polish).** Workspace version bump from `2.0.0-alpha.2` to `2.0.0-beta.1` across **6 lockstep manifests** (Cargo.toml workspace + root `package.json` (HIGH-1 reviewer fix added root to lockstep) + 4 sub-manifests: atlas-bridge + atlas-cypher-validator + atlas-mcp-server + atlas-web) + Cargo.lock auto-regenerated + CHANGELOG `[Unreleased]` → `[2.0.0-beta.1] — 2026-05-15` conversion with V2-β tripod ship summary + NEW `docs/V2-BETA-1-RELEASE-NOTES.md` (~145 lines, 13 sections, scaffold-posture LOUDLY stated) + NEW `docs/SEMVER-AUDIT-V2.0-beta.md` (~135 lines, 8 sections, mirrors V1.0 methodology) + README badge alt-text update (M-2 polish PR #103). Parallel reviewer dispatch (Lesson #8) on PR #102: **0 CRITICAL + 1 HIGH (root `package.json` lockstep) + 3 MEDIUM (MCP SDK introspection + SEMVER-AUDIT draft artifact + cargo semver clarity) + 1 LOW (CHANGELOG self-reference)** all applied in-commit (fix-commit `1789e58` on top of initial `8287b97`). PR #103 polish reviewer dispatch: **6/6 PASS APPROVE both reviewers** — M-1 Release Notes GDPR phrasing + M-2 README badge alt-text + L-2 Phase D parenthetical (combined with M-1), all from the multi-perspective pre-tag review and landed BEFORE tag-push. **Post-merge ship cascade (parent-led):** signed tag `v2.0.0-beta.1` SHA `81d363e58eb9ec6b5234d1f4c4c091683e754a17` Good Ed25519 (`nelson@ultranova.io` fingerprint `SHA256:qq/VVJYpsgEdeQSLqU0QS/gKn6ohXJHio+VkzVX+4Zg`); GitHub Release prerelease https://github.com/ThePyth0nKid/atlas/releases/tag/v2.0.0-beta.1; npm `@atlas-trust/verify-wasm@2.0.0-beta.1` LIVE with dist-tags `latest = 2.0.0-beta.1` + `node = 2.0.0-beta.1`; Sigstore Build L3 provenance attached shasum `60d9160d43e3e4de89a236b40ee584522b020c56`; wasm-publish.yml W11 race-fix validated **2nd time end-to-end** (Phase 3 V2-α-α.2 was 1st). Byte-pin `8962c1681a44f9569f78c5917f568c5a027ac69f727f23ba5e8f871e5e013ac4` reproduces unchanged through both InMemoryBackend AND ArcadeDbBackend; 577 tests pass; clippy `-D warnings` zero; tag-immutability per V1.17 Welle B preserved. Layer 3 SCAFFOLD-SHIPPED with W18c-A pins lifted (semantic-search Read-API returns 501 stub until W18c Phase B `try_new_from_user_defined` activates); scaffold-honesty LOUDLY stated in release notes. v2.0.0-beta.1 tag is **internal engineering milestone** per `DECISION-COMPLIANCE-3` / `DECISION-COUNSEL-1` (NOT V2-β public material; counsel-track blocks public materials but not the tag itself).

**Phase 14.6 SHIPPED 2026-05-15:** Phase-14 W19 consolidation commit (PR #104 `741bcde`) — CHANGELOG `[Unreleased]` Phase-14.6 entry + master-plan §6 status flip (Phase 14 W19 SHIPPED row + Phase 14.6 SHIPPED row; "Next welle" pivots to W18c Phase B as primary engineering-pipeline path post-v2.0.0-beta.1-LIVE) + decisions.md `DECISION-ARCH-W19` entry (29th decision) + orchestration-plan Phase 14 + Phase 14.6 status flips + dependency-graph W19 terminus + Phase 14.6 SHIPPED node + W18c Phase B next-target + handoff §0z7 W19 SHIPPED narrative + §0-NEXT refresh (W18c Phase B as primary; NEW Lesson #19 pre-tag-push multi-perspective review codified).

**Phase 14 W18c Phase B SHIPPED 2026-05-15:** **W18c Phase B — fastembed `try_new_from_user_defined` wiring — Layer 3 embedder OPERATIONAL (PR #105 `76046be`).** Initial commit `826d3e2c` (SSH-Ed25519 signed by Phase B subagent) + reviewer fix-commit `0563ffa7a2c27ef421232318338f0e068f2323b9` on top per Lesson #3, squash-merged. 7 files changed; +1046/-404 lines; NEW `crates/atlas-mem0g/src/supply_chain.rs` (~552 LOC extracted module containing 11 compile-in pins + NEW `read_and_verify(path, expected_sha) -> Mem0gResult<Vec<u8>>` single-buffer TOCTOU-free primitive). **Critical Phase B discovery:** fastembed 5.13.4's `TokenizerFiles` struct surfaced `tokenizer_config_file` as a 4th required field during Step 0 API verification; pin set atomically extended **9 → 11** with NEW `TOKENIZER_CONFIG_JSON_SHA256 = 9261e7d79b44c8195c1cada2b453e55b00aeb81e907a6664974b4d7776172ab3` + matching URL (revision-pinned `5c38ec7c…`). Total: 1 × SHA-1 + 5 × SHA-256 + 5 × URL. **Architectural choices:** `Mutex<TextEmbedding>` interior-mutability wrapper (fastembed `embed(&mut self)` requires); `Pooling::Cls` per BGE-Small-EN-v1.5 upstream default (matches `get_default_pooling_method(BGESmallENV15)`). **TOCTOU eliminated** via `read_and_verify` — single contiguous `Vec<u8>` is BOTH hashed AND fed to fastembed; bytes hashed ARE bytes consumed. Reviewer dispatch (Lesson #8): code-reviewer REJECT (APPROVE-WITH-FIXES) — 0 CRITICAL / 2 HIGH (one **DISMISSED** via Lesson #2 behavioural-test settlement — claim that `TOKENIZER_CONFIG_JSON_SHA256` was 63 chars verified WRONG via parent `echo -n | wc -c = 64` + independent HF `curl|sha256sum` byte-identical match; one applied — missing reqwest timeout) / 4 MEDIUM (all applied) / 3 LOW (1 applied); security-reviewer APPROVE-WITH-FIXES — 0 CRITICAL / 1 HIGH TOCTOU verify-then-use (applied via `read_and_verify`) / 2 MEDIUM (applied) / 3 LOW (deferred to Phase C/D + operator-runbook welle). All applied in fix-commit `0563ffa7a` on top of initial `826d3e2c`. **Posture preserved:** W18b HIGH-2 fix (`try_new_from_user_defined` exclusive; no `Default::default()` bypass) preserved + grepped clean by security-reviewer; trait surface SemVer-locked; byte-pin `8962c1681a44…e013ac4` reproduces through both InMemoryBackend AND ArcadeDbBackend; embeddings OUTSIDE canonicalisation pipeline (lib.rs invariant #3 honoured); Layer 3 NEVER trust-authoritative (cite-back trust intact). **579 tests pass workspace-wide / 0 failed / 7 ignored** (delta +2 vs pre-Phase-B 577: 2 new `read_and_verify` match/mismatch behavioural tests); clippy `-D warnings` zero workspace-wide + `cargo check -p atlas-mem0g --features lancedb-backend` clean. CI required-checks: `Verify trust-root` ✓ + `atlas-web-playwright` ✓ (4m31s) + `atlas-mem0g-smoke` ✓ (1m03s — first end-to-end Layer-3-operational validation against live HF infra: model download + 6 SHA-256 verified + fastembed init + 384-dim FP32 vector emitted). `atlas-mem0g-smoke` is now a **promotion-to-required-check candidate** (3rd consecutive stable run: W18b initial + W18c-A + W18c-B). `/api/atlas/semantic-search` Read-API STILL returns 501 stub — Phase D (LanceDB ANN/search body fill-in) is separate scope; Phase B activates the embedder, Phase D activates the storage+retrieval.

**Phase 14.7 SHIPPED 2026-05-15:** Phase-14 W18c Phase B consolidation commit (THIS PR) — CHANGELOG `[Unreleased]` Phase-14.7 entry + master-plan §6 status flip (Phase 14 W18c-B SHIPPED row + Phase 14.7 SHIPPED row; "Next welle" pivots to W18c Phase C OR Phase D as engineering-pipeline-parallel-trackable post-Phase-B-SHIPPED) + decisions.md `DECISION-ARCH-W18c-B` entry (31st decision) + orchestration-plan Phase 14 W18c-B + Phase 14.7 status flips + dependency-graph W18c-B SHIPPED node + Phase 14.7 SHIPPED node + W18c-C/D next-target downstream + handoff §0z8 W18c Phase B SHIPPED narrative + §0-NEXT refresh (W18c Phase C OR Phase D as engineering-pipeline-parallel-trackable; mem0g-smoke promotion-candidate threshold met flag).

**Next welle (Phase 14 SHIPPED — W18c-A + W18c-B + W19 + Phase 14.5 + Phase 14.6 + Phase 14.7 all SHIPPED):** **W18c Phase C (V1-V4 verification gap CI matrix — Linux + Windows + macOS for fastembed determinism + Lance v2.2 `_deletion_files` + LanceDB Windows `cleanup_old_versions`)** OR **W18c Phase D (LanceDB ANN/search body fill-in via `tokio::task::spawn_blocking`-wrapped LanceDB calls per spike §7; NEVER `Handle::current().block_on()` — deadlocks)** — both engineering-pipeline parallel-trackable post-Phase-B-SHIPPED; **Phase D is the welle that pivots `/api/atlas/semantic-search` to real hits** (replaces 501 stub). ~1 session agent-only (Phase C) or ~1-2 sessions (Phase D). ADR-Atlas-013 reserved for W18c Phase C/D design amendments. **`atlas-mem0g-smoke` promotion-to-required-check** queued cosmetic-eligible operator-decision (3-stable-run threshold MET as of Phase B; NOT in any imminent welle scope). **Counsel-engagement firm selection + outreach kickoff** continues parallel-track Nelson-led per `DECISION-COUNSEL-1` — now MORE valuable post-v2.0.0-beta.1-LIVE + Layer-3-embedder-OPERATIONAL (concrete artefact with end-to-end functional embedder demonstrable against live HuggingFace infra).
