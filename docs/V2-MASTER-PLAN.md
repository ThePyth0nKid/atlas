...
**Phase 10 next:** W17b ArcadeDB driver implementation. SERIAL `general-purpose` subagent. Fills the `ArcadeDbBackend` stub with `reqwest`-based HTTP calls to ArcadeDB's `/api/v1/begin/{db}` + Cypher endpoints per ADR-Atlas-010 §4. Must pass cross-backend byte-determinism test (`InMemoryBackend::canonical_state()` byte-identical to `ArcadeDbBackend::canonical_state()` for the same `events.jsonl`). W17a plan-doc carry-over section lists 4 deferred MEDIUMs W17b must lift in: (1) `serde_json::Value` depth cap when parsing ArcadeDB HTTP responses; (2) `WorkspaceId` validation guard before HTTP request construction; (3) `begin()` lifetime `'_` vs `'static` evaluation BEFORE the trait change becomes SemVer-breaking; (4) ProjectorError variant naming defer-to-V2-γ. Then Phase 11 = W17c Docker-Compose CI + integration tests + benchmark capture (replaces ADR-010 §4.10 estimates with measured numbers; embedded-mode reconsideration trigger at p99 > 15 ms).
