...
**Phase 13 next:** W18b Mem0g Layer-3 cache **implementation** (~1500-2000 LOC: NEW `crates/atlas-mem0g/` + `apply_embedding_erased` dispatch arm in `crates/atlas-projector/src/upsert.rs` + cross-platform determinism test + secure-delete-correctness + race-test + B4/B5/B6 benches + new CI workflow `.github/workflows/atlas-mem0g-smoke.yml` + Read-API endpoint `apps/atlas-web/src/app/api/atlas/semantic-search/route.ts`). Ready-to-dispatch subagent skeleton in `.handoff/v2-beta-welle-18-plan.md`. Then Phase 14 = W19 v2.0.0-beta.1 ship convergence milestone.
