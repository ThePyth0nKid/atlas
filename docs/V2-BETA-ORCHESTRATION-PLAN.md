...
> - Phase 12.5 SHIPPED — this consolidation commit (CHANGELOG + master-plan §6 + decisions DECISION-ARCH-W18 entry + dependency-graph status flip + handoff §0z4 W18 SHIPPED narrative + §0-NEXT refresh to W18b entry).
> - Phase 13 next — W18b Mem0g implementation (~1500-2000 LOC: NEW `crates/atlas-mem0g/` + `apply_embedding_erased` dispatch arm + cross-platform determinism test + secure-delete-correctness + race-test + B4/B5/B6 benches + new CI workflow + Read-API endpoint with response-time normalisation). Ready-to-dispatch subagent skeleton in `.handoff/v2-beta-welle-18-plan.md`. Then Phase 14 = W19 v2.0.0-beta.1 ship convergence milestone.
