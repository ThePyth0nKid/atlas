...
> - Phase 13.5 SHIPPED — this consolidation commit (CHANGELOG + master-plan §6 + decisions `DECISION-ARCH-W18b` entry + dependency-graph status flip + handoff §0z5 W18b SHIPPED narrative + §0-NEXT refresh to W19 ship convergence with W18c parallel-track note).
> - Phase 14 next — **W19 v2.0.0-beta.1 ship convergence milestone.** Layer 2 ArcadeDB + Layer 3 Mem0g (scaffold) + verifier-rebuild all operational on master; workspace version bump from `2.0.0-alpha.2` to `2.0.0-beta.1`; signed tag `v2.0.0-beta.1` + GitHub Release + npm publish `@atlas-trust/verify-wasm@2.0.0-beta.1` (analog V2-α-α.2 ship pattern from W11).
> - W18c (queued, parallel-track, pre-V2-β-1-ship blocker, NOT in numbered phase sequence): Nelson supply-chain constant lift (real `HF_REVISION_SHA` + `ONNX_SHA256` + `MODEL_URL` from HuggingFace `BAAI/bge-small-en-v1.5`) + 3 additional tokenizer-file SHA-256 pins (tokenizer.json + config.json + special_tokens_map.json) + fastembed `try_new_from_user_defined` wiring + close V1-V4 verification gaps from spike §12 + lift LanceDB ANN/search body stubs. ADR-Atlas-013 reserved.
