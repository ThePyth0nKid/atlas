...
> - Phase 10.5 SHIPPED — this consolidation commit (CHANGELOG + master-plan §6 + decisions Welle-17 closure rows + dependency-graph status flip + handoff §0z2 W17b SHIPPED narrative).
> - Phase 11 next — W17c Docker-Compose CI workflow + actual cross-backend byte-determinism run against live ArcadeDB sidecar + benchmark capture.
