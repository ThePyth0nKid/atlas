...
### Added — V2-β Welle 20c (atlas-web settings UI + L3 honest status + orphan POST rollback, 2026-05-16)

- **NEW `apps/atlas-web/src/app/settings/page.tsx` + `<SettingsContent>`** — `/settings` route with four panels: workspace list (rename + delete with typed-confirmation), 11-pin supply-chain panel (read-only mirror of `crates/atlas-mem0g/src/embedder.rs`), signer/embedder/backend status (consumes `/api/atlas/system/health`), V2-γ coming-soon placeholders.
- **NEW `GET /api/atlas/system/health`** — env-only L3 probe with 60-second TTL cache. Returns `{ signer, embedder, backend }` discriminated unions. Powers `<LayerStatusPanel>` (dashboard) + `<SignerStatusPanel>` (/settings). Test-hook headers honored only when `ATLAS_E2E_TEST_HOOKS=1` in server env (set by `playwright.config.ts` only).
- **NEW `GET /api/atlas/system/supply-chain-pins`** — returns the 11 compile-in supply-chain pins (1 × Git SHA-1, 5 × SHA-256, 5 × HuggingFace URLs). Parity smoke in `supply-chain-pins.test.ts` traps drift against `embedder.rs`.
- **NEW `<LayerStatusPanel>` (dashboard, above `<LiveVerifierPanel>`)** — three pills (signer/embedder/backend) reading the health endpoint. Frozen-testid contract: `layer-status-panel`, `layer-status-signer`, `layer-status-embedder`, `layer-status-backend`. `<LiveVerifierPanel>` testids unchanged per Lesson #19.
- **MODIFY `DashboardMetricsSection`** — accepts optional `layerStatus` prop. DA-4: tier resolution is now `min(eventCountTier, layerReadinessTier)`. A workspace with 200 events but `signer !== 'operational'` renders `<EmptyTier>` with the new `dashboard-layer-not-ready` banner pointing to `/settings`. Existing tier testids preserved.
- **MODIFY `POST /api/atlas/workspaces`** — DA-1 atomic rollback on signer failure. The `derivePubkeyViaSigner` post-mkdir branch now `fs.rm`'s the freshly-created workspace directory before returning 500. Eliminates the orphan "unconfigured" workspace UX introduced by ba4e27f (W20b-2) and flagged by security-reviewer in PR #113. If `fs.rm` itself fails, surfaces `partial_rollback:` prefix so operators can clean up by hand.
- **NEW `PATCH /api/atlas/workspaces`** (DA-2) — rename via atomic `fs.rename` after validating both old and new ids. Re-derives the per-tenant pubkey under the new id. EXDEV (cross-mount) surfaces as `500 cross_mount_rename_unsupported`. 409 on target collision; 404 on missing source.
- **NEW `DELETE /api/atlas/workspaces`** (DA-6) — typed-confirmation client gate + server-side 409 when the delete would remove the last user-facing workspace. Belt-and-braces against flipping the UI into wizard mode mid-session.
- **MODIFY `WorkspaceContext`** — `renameWorkspace` + `deleteWorkspace` methods; pure helpers `requestRenameWorkspace` / `requestDeleteWorkspace` for unit testability. localStorage re-pinned on rename of active workspace; cleared on delete of active workspace.
- **MODIFY `apps/atlas-web/src/app/layout.tsx`** — `/settings` link added to the NAV array between core surfaces and coming-soon entries.
- **NEW e2e specs** — `settings.spec.ts` (renders + rename happy/invalid + delete typed-confirm + supply-chain pin count + signer status), `orphan-workspace.spec.ts` (invalid-id 400 + happy-path POST/GET roundtrip; signer-failure rollback covered exhaustively in `route.test.ts`), `layer-status-panel.spec.ts` (DOM-tree audit + DA-4 degradation via `forceSignerUnconfigured` fixture).
- **Lesson #26 sweep** — added `layer-status-panel` visibility anchor assertions to `dashboard-tiers.spec.ts` + `a11y.spec.ts`; added `layer-status-panel` absence assertion to `first-run-wizard.spec.ts`.
- **NEW e2e fixtures** — `provisionAndSelectMany(page, baseId, count)` for rename/delete tests; `forceSignerUnconfigured(page)` for DA-4 degradation tests (honors header only when `ATLAS_E2E_TEST_HOOKS=1`).
- **MODIFY `playwright.config.ts`** — port now configurable via `ATLAS_WEB_PORT` env-var (defaults to 3001); `ATLAS_E2E_TEST_HOOKS=1` added to webServer env so test-hook headers are honored only for the test process.
- **MODIFY `docs/OPERATOR-RUNBOOK.md`** — new §18 "Signer health probe + L3 status surface" documenting the new endpoints, status enum values, DA-4 degradation contract, cache TTL behaviour, test-hook production safety, and recovery procedures for each failure mode.

Architecture decisions (all locked in `.handoff/v2-beta-welle-20c-plan.md` §2): DA-1 rollback-not-retry, DA-2 PATCH-with-fs.rename, DA-3 new health route, DA-4 min(eventCount, layerReadiness), DA-5 frozen LiveVerifierPanel testids + new `settings-*` + `layer-status-*` prefixes, DA-6 typed-confirmation + server-side last-workspace gate. Single squash-merged PR per W20b-2 cascade pattern.

**Post-v2.0.0-beta.1 ship consolidation + Layer 3 embedder OPERATIONAL milestone.** v2.0.0-beta.1 LIVE on npm + GitHub + master (signed tag `81d363e58eb9ec6b5234d1f4c4c091683e754a17` Good Ed25519; GitHub Release prerelease https://github.com/ThePyth0nKid/atlas/releases/tag/v2.0.0-beta.1; npm dist-tags `latest = 2.0.0-beta.1` + `node = 2.0.0-beta.1`; Sigstore Build L3 provenance attached, shasum `60d9160d43e3e4de89a236b40ee584522b020c56`; W11 wasm-publish.yml race-fix validated 2nd time end-to-end, run-id `25919934805`). **W18c Phase B SHIPPED 2026-05-15 — Layer 3 embedder OPERATIONAL** (PR #105 `76046be` master commit; initial `826d3e2c` + fix-commit `0563ffa7a2c27ef421232318338f0e068f2323b9`): `fastembed::TextEmbedding::try_new_from_user_defined` wired against 11 compile-in supply-chain pins (atomic 9 → 11 extension with NEW `TOKENIZER_CONFIG_JSON_SHA256` 4th tokenizer pin per fastembed 5.13.4 `TokenizerFiles` struct discovery); `AtlasEmbedder::new` returns `Ok(_)` against real bge-small-en-v1.5 model dir; `embed()` returns 384-dim FP32 vector verified end-to-end via live `atlas-mem0g-smoke` CI in 1m03s; TOCTOU eliminated via NEW `read_and_verify` single-buffer primitive (bytes hashed ARE bytes consumed); `Mutex<TextEmbedding>` interior-mutability wrapper + `Pooling::Cls` per BGE upstream default; W18b HIGH-2 fix preserved. **BUT `/api/atlas/semantic-search` Read-API STILL returns 501 stub** — Phase D LanceDB ANN/search body fill-in is the next gate that activates real hits. Phase 14.7 cross-doc state flip (THIS PR) consolidates the Phase B SHIPPED state across handoff §0z8 + decisions `DECISION-ARCH-W18c-B` (31st) + master-plan §6 + orchestration + dependency-graph. Next engineering-pipeline welles: W18c Phase C (V1-V4 cross-platform CI matrix) OR W18c Phase D (LanceDB ANN/search body fill-in) — both agent-only, engineering-pipeline-parallel-trackable. NEW Atlas Standing Protocol Lesson #19 (pre-tag-push multi-perspective review with 4 parallel reviewers is load-bearing for irreversible-public-action ship events) codified in Phase 14.6.

### Docs — V2-β Phase 14.7 (W18c Phase B SHIPPED consolidation, 2026-05-15)

- **MODIFY `.handoff/v2-session-handoff.md`** — NEW §0z8 W18c Phase B SHIPPED narrative (analog §0z7 W19 structure: what landed in PR #105, W18c Phase B critical discovery (fastembed 5.13.4 `TokenizerFiles` struct requires 4 tokenizer files not 3; pin set atomically extended 9 → 11 with NEW `TOKENIZER_CONFIG_JSON_SHA256 = 9261e7d79b44…`), reviewer-dispatch outcome aggregate (code-reviewer REJECT-with-fixes 0/2 HIGH with one DISMISSED via Lesson #2 behavioural-test settlement + security-reviewer APPROVE-with-fixes 0/1 HIGH TOCTOU applied via `read_and_verify` single-buffer primitive), Phase B architectural choices (Mutex<TextEmbedding> interior mutability + Pooling::Cls per BGE upstream default + supply_chain.rs module split), Phase B posture preserved (W18b HIGH-2 fix preserved + trait surface SemVer-locked + byte-pin reproduces + TOCTOU eliminated + embeddings OUTSIDE canonicalisation pipeline + Layer 3 NEVER trust-authoritative + Hermes-skill npx distribution intact + fail-closed posture for any future drift), behavioural-test settlement narrative (code-reviewer HIGH-1 dismissal via parent `echo -n | wc -c = 64` + independent HF `curl|sha256sum` byte-identical match + in-tree behavioural test `pins_remain_well_formed_post_phase_a_lift` len()==64 passes unmodified — three independent signals refute reviewer claim per Lesson #2), CI required-checks status (all green; `atlas-mem0g-smoke` 3rd consecutive stable run promotion-to-required-check candidate threshold met), what's next pivoting to W18c Phase C OR Phase D as engineering-pipeline-parallel-trackable post-Phase-B-SHIPPED + counsel kickoff Nelson-led); §0-NEXT 5-min-snapshot refresh ("V2-β Phase 0-14.7 all SHIPPED + v2.0.0-beta.1 LIVE + Layer 3 embedder OPERATIONAL"; active-welle line refresh from "W18c Phase B fastembed wiring as primary engineering-pipeline path post-v2.0.0-beta.1-LIVE" to "W18c Phase C V1-V4 verification gap CI matrix OR W18c Phase D LanceDB ANN/search body fill-in — both engineering-pipeline parallel-trackable; Phase D activates `/api/atlas/semantic-search` real hits"; Layer 3 status line "SCAFFOLD-SHIPPED with W18c Phase A supply-chain pins lifted" → "Embedder OPERATIONAL with Phase B fastembed wiring SHIPPED"); §0-NEXT Pre-flight checklist updated (master HEAD post-Phase-14.7 with `git log` top-3 reflecting PR #105 + PR #104 + PR #103; `cargo test --workspace` expected count updated to 579 / 0 failed / 7 ignored from 577 — delta +2 vs pre-Phase-B for 2 new `read_and_verify` match/mismatch behavioural tests in `supply_chain.rs`; NEW `cargo check -p atlas-mem0g --features lancedb-backend` check added per Phase B feature-on contract preservation); §0-NEXT blocked-on-Nelson list refresh (`atlas-mem0g-smoke` promotion-to-required-check candidate added as cosmetic-eligible operator-decision; 3-stable-run threshold met as of Phase B but promotion separate operator-decision NOT in Phase 14.7 scope).
- **MODIFY `.handoff/decisions.md`** — NEW `DECISION-ARCH-W18c-B` entry (31st decision; documents Phase B implementation posture (Layer 3 embedder OPERATIONAL with `AtlasEmbedder::new` returning `Ok(_)` against real bge-small-en-v1.5 model dir + 384-dim embed() output; `/api/atlas/semantic-search` Read-API STILL returns 501 stub pending Phase D) + critical Phase B discovery (4-tokenizer-file atomic pin set extension 9 → 11 with NEW `TOKENIZER_CONFIG_JSON_SHA256`) + Phase B architectural choices (Mutex<TextEmbedding> + Pooling::Cls + supply_chain.rs module split) + TOCTOU elimination via `read_and_verify` single-buffer primitive (security-reviewer HIGH-1 closure) + reviewer-dispatch outcome aggregate including code-reviewer HIGH-1 behavioural-test dismissal per Lesson #2 + Phase B posture preserved (W18b HIGH-2 fix + trait surface SemVer + byte-pin + lib.rs invariant #3 + cite-back trust property + Hermes-skill `npx` + fail-closed posture for drift) + supply-chain pin contract (11 total = 1 × SHA-1 + 5 × SHA-256 + 5 × URL; atomic-lift contract preserved; future rotations re-runnable via auditable resolver helper extended for 4 tokenizer files) + CI status post-merge (all required green; `atlas-mem0g-smoke` promotion-to-required-check candidate 3-stable-run threshold met) + SemVer impact (additive within atlas-mem0g 0.1.0; AtlasEmbedder still Internal-deferred-to-V2-γ-stability per `docs/SEMVER-AUDIT-V2.0-beta.md`; v2.0.0-beta.1 tag remains valid + forward-version SemVer-patch v2.0.0-beta.2 is recovery path for any Phase C/D-surfaced amendment) + reversibility MED-HIGH (code revertable; pin set forward-only) + review-after W18c Phase D ship event). Decisions counter advanced to 31.
- **MODIFY `docs/V2-MASTER-PLAN.md`** — §6 Welle Decomposition: NEW "Phase 14 W18c Phase B SHIPPED 2026-05-15" row documenting the fastembed wiring + Layer 3 embedder OPERATIONAL milestone (initial commit + fix-commit SHAs + 7 files / +1046/-404 / NEW `supply_chain.rs` module split + atomic 9→11 pin set extension + Mutex<TextEmbedding>/Pooling::Cls architectural choices + TOCTOU elimination + reviewer dispatch aggregate including code-reviewer HIGH-1 dismissal via Lesson #2 + posture preserved + 579 tests + CI required-checks all green + mem0g-smoke promotion-candidate) and NEW "Phase 14.7 SHIPPED" row (this consolidation); "Next welle" pivots from "W18c Phase B fastembed wiring as primary engineering-pipeline path post-v2.0.0-beta.1-LIVE" to "W18c Phase C (V1-V4 verification gap CI matrix) OR W18c Phase D (LanceDB ANN/search body fill-in)" — both engineering-pipeline-parallel-trackable post-Phase-B-SHIPPED + Phase D pivots `/api/atlas/semantic-search` to real hits; `atlas-mem0g-smoke` promotion-candidate flag added as queued cosmetic-eligible operator-decision NOT in Phase 14.7 scope.
- **MODIFY `docs/V2-BETA-ORCHESTRATION-PLAN.md`** — Phase 14 W18c-B SHIPPED status block (mirrors Phase 14 W19 SHIPPED structure: PR #105 commit SHAs + reviewer-dispatch aggregate including code-reviewer HIGH-1 dismissal + architectural choices + posture preserved + CI status all green + mem0g-smoke promotion-candidate threshold met); NEW Phase 14.7 SHIPPED status block (this consolidation); "Next welle (Phase 15)" pointer updated from "W18c Phase B fastembed wiring" to "W18c Phase C V1-V4 verification gap CI matrix OR W18c Phase D LanceDB ANN/search body fill-in"; cross-batch consistency-reviewer dispatch fulfilled per §3.5 invariant for Phase 14 noted; `atlas-mem0g-smoke` promotion-to-required-check candidate flagged as queued cosmetic-eligible operator-decision NOT in Phase 14.7 scope.
- **MODIFY `docs/V2-BETA-DEPENDENCY-GRAPH.md`** — W18c Phase B SHIPPED node (mermaid + textual): style flipped from `fill:#ffffcc` queued to `fill:#ccffcc` SHIPPED; node label expanded with shipping artefact details (Layer 3 embedder OPERATIONAL + 11 compile-in pins atomic 9→11 + read_and_verify TOCTOU-free primitive + Mutex<TextEmbedding>/Pooling::Cls + 579 tests/clippy zero/mem0g-smoke 1m03s live HF + reviewer dispatch aggregate including code-reviewer HIGH-1 dismissal); NEW Phase 14.7 SHIPPED consolidation node (mermaid + textual; edges W18c-B → Phase 14.7; analog Phase 14.6 placement); W18c Phase C + Phase D queued as engineering-pipeline parallel-track downstream of Phase B (Phase D activates `/api/atlas/semantic-search` real hits replacing 501 stub); §2 textual edges updated (W18c-A → W18c-B SHIPPED + W18c-B → W18c-C → W18c-D edge chain); §4 ADR-013 row updated to reflect Phase A + Phase B both SHIPPED (Phase C + Phase D pending); §5 Critical-path analysis updated (13 nodes ALL SHIPPED as of 2026-05-15; longest serial path extended with W18c-B → Phase 14.7; Layer 3 embedder OPERATIONAL post-Phase-B-SHIPPED noted; post-Phase-B-SHIPPED engineering-pipeline pivots to Phase C OR Phase D with Phase D activating semantic-search real hits).
- **Strategic rationale:** Phase 14.7 mirrors the Phase 12.5 / 13.5 / 13.6 / 14.5 / 14.6 pattern. Cross-doc state stays consistent so a fresh agent entering at any of `.handoff/v2-session-handoff.md` / `decisions.md` / `V2-MASTER-PLAN.md` / `V2-BETA-ORCHESTRATION-PLAN.md` / `V2-BETA-DEPENDENCY-GRAPH.md` sees the same current-state snapshot (v2.0.0-beta.1 LIVE end-to-end + Layer 3 embedder OPERATIONAL + W18c Phase C OR Phase D as next engineering-pipeline-parallel-trackable welles + `atlas-mem0g-smoke` promotion-candidate flag). Code touched: zero (administrative consolidation; CI risk LOW; reversibility HIGH). Atlas-web-playwright triggered via `.handoff/**` touch per Atlas Standing Protocol Lesson #11.

### Docs — V2-β Phase 14.6 (W19 SHIPPED consolidation, 2026-05-15)

- **MODIFY `.handoff/v2-session-handoff.md`** — NEW §0z7 W19 SHIPPED narrative (analog §0z6 W18c Phase A structure: what landed in PR #102 + PR #103, W19 reviewer-dispatch outcome aggregate, post-merge ship cascade verification (signed tag SHA + GitHub Release URL + npm registry state + Sigstore provenance shasum + wasm-publish.yml run outcome), W19 posture preserved, multi-perspective pre-tag review with 4 perspectives all GO, what's next pivoting to W18c Phase B); §0-NEXT 5-min-snapshot refresh ("V2-β Phase 0-14.6 all SHIPPED + v2.0.0-beta.1 LIVE end-to-end" + active-welle line refresh from "W19 ship convergence primary OR W18c Phase B alternate" to "W18c Phase B fastembed wiring as primary engineering-pipeline path post-v2.0.0-beta.1-LIVE"); §0-NEXT Pre-flight checklist updated to expect master HEAD post-Phase-14.6 with `git verify-tag v2.0.0-beta.1` Good signature; NEW Atlas Standing Protocol Lesson #19 added to lesson catalogue.
- **MODIFY `.handoff/decisions.md`** — NEW `DECISION-ARCH-W19` entry (29th decision; documents W19 ship convergence + post-tag polish PR #103 + reviewer-dispatch outcomes 0/1/3/1 on PR #102 + 6/6 PASS on PR #103 + multi-perspective pre-tag review (4 reviewers all GO) + post-merge ship cascade verification including signed tag SHA + Sigstore provenance shasum + npm dist-tag state + W11 race-fix validation 2nd time). Decisions counter advanced to 29.
- **MODIFY `docs/V2-MASTER-PLAN.md`** — §6 Welle Decomposition: NEW "Phase 14 W19 SHIPPED 2026-05-15" row documenting the ship convergence event (6 lockstep manifests + CHANGELOG conversion + V2-BETA-1-RELEASE-NOTES + SEMVER-AUDIT-V2.0-beta companion + README polish + post-merge ship cascade) and NEW "Phase 14.6 SHIPPED" row (this consolidation); "Next welle" pivots from "W19 ship convergence primary OR W18c Phase B alternate" to "W18c Phase B fastembed wiring as PRIMARY engineering-pipeline path post-v2.0.0-beta.1-LIVE" with Phase C/D follow-ons + parallel-track counsel kickoff Nelson-led.
- **MODIFY `docs/V2-BETA-ORCHESTRATION-PLAN.md`** — Phase 14 W18c-A + Phase 14.5 + Phase 14 W19 + Phase 14.6 status block expansion; "Next welle (Phase 15 = W18c Phase B)" pointer added; cross-batch consistency-reviewer dispatch for Phase 14 noted per §3.5 invariant.
- **MODIFY `docs/V2-BETA-DEPENDENCY-GRAPH.md`** — W19 SHIPPED terminus node (with shipping artefact details: signed tag SHA + GitHub Release + npm dist-tags + Sigstore shasum); NEW Phase 14.6 SHIPPED consolidation node; W18c Phase B + Phase C + Phase D queued as engineering-pipeline parallel-track downstream of W19; W18b tests label corrected from 578 → 577 (post-W18c-A retirement of `pins_are_placeholder_until_nelson_verifies` gatekeeper); §5 Critical-path analysis updated (11 nodes ALL SHIPPED as of 2026-05-15).
- **Strategic rationale:** Phase 14.6 mirrors the Phase 12.5 / 13.5 / 13.6 / 14.5 pattern. Cross-doc state stays consistent so a fresh agent entering at any of `.handoff/v2-session-handoff.md` / `decisions.md` / `V2-MASTER-PLAN.md` / `V2-BETA-ORCHESTRATION-PLAN.md` / `V2-BETA-DEPENDENCY-GRAPH.md` sees the same current-state snapshot (v2.0.0-beta.1 LIVE end-to-end + W18c Phase B as next primary welle). Code touched: zero (administrative consolidation; CI risk LOW; reversibility HIGH). Atlas-web-playwright triggered via `.handoff/**` touch per Atlas Standing Protocol Lesson #11.
