...
- **ADD three Phase-B-prep constants in same file** — `TOKENIZER_JSON_SHA256` / `CONFIG_JSON_SHA256` / `SPECIAL_TOKENS_MAP_SHA256` (64-char SHA-256 each, consumed by W18c Phase B `fastembed::TextEmbedding::try_new_from_user_defined` wiring) plus matching `TOKENIZER_JSON_URL` / `CONFIG_JSON_URL` / `SPECIAL_TOKENS_MAP_URL` (revision-pinned huggingface.co LFS URLs). Atomic lift across all six SHAs + four URLs.
- **`_STRUCTURAL_PIN_CHECK` const block extended** to assert non-emptiness of all nine compile-in pins.
- **Test `pins_are_placeholder_until_nelson_verifies` retired** (W18b gatekeeper purpose served); **`pins_well_formed_after_lift` upgraded** to unconditional structural enforcement across all 6 SHAs + 4 URLs (length + lowercase-hex + `https://huggingface.co/` origin + revision-SHA-in-path invariants). The W18b `is_placeholder` early-return is removed; any future refactor that reintroduces placeholder strings trips the assertions at test time.
