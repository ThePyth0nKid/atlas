# Atlas V2 — Session Handoff (V2-α SHIPPED + V2-β Phase 0–14.7 SHIPPED, v2.0.0-beta.1 LIVE + Layer 3 embedder OPERATIONAL)

> **🎯 FRESH-AGENT BOOTSTRAP DOC.** **READ §0-NEXT FIRST** (2026-05-15-after-Phase-14.7 next-session entry — 5-min snapshot + **W18c Phase C (V1-V4 verification gap CI matrix) OR W18c Phase D (LanceDB ANN/search body fill-in)** as primary engineering-pipeline paths post-Phase-B-SHIPPED + Nelson-only items). Then §0z8 (Phase 14 W18c Phase B fastembed wiring — Layer 3 embedder OPERATIONAL — SHIPPED narrative, 2026-05-15), §0z7 (Phase 14 W19 v2.0.0-beta.1 ship convergence SHIPPED narrative, 2026-05-15), §0z6 (Phase 14 W18c Phase A supply-chain constants lifted SHIPPED narrative, 2026-05-15), §0z5 (Phase 13 W18b Mem0g implementation SHIPPED narrative, 2026-05-14), §0z4 (Phase 12 W18 Phase A Mem0g design SHIPPED narrative, 2026-05-15), §0z3 (Phase 11 W17c SHIPPED narrative, 2026-05-14 late-day), §0z2 (Phase 10 W17b SHIPPED narrative, 2026-05-14), §0 "Fresh-Context Onboarding", §0z (V2-β Phase 0–9.5 SHIPPED, 2026-05-13), §0-NOW (HISTORICAL: 2026-05-14 Docker-restart breakpoint resume), §0a–§0d (Phase 1–4 strategic-iteration SHIPPED, historical). Then **`docs/V2-MASTER-PLAN.md`** + **`docs/V2-BETA-ORCHESTRATION-PLAN.md`** + **`docs/V2-BETA-DEPENDENCY-GRAPH.md`** + **`docs/V2-BETA-MEM0G-SPIKE.md`** + **`docs/ADR/ADR-Atlas-012-mem0g-layer3-design.md`**. Optional: **`.handoff/v2-master-vision-v1.md`** + **`.handoff/decisions.md`** (31 explicit decisions).

**Erstellt:** 2026-05-12. **V2-α-α.1 SHIPPED:** 2026-05-13 (8 Welles). **V2-β Phase 0–9.5 SHIPPED:** 2026-05-13 (18 PRs merged: #67-#86). **V2-β Phase 10-counsel + 10-cleanup SHIPPED:** 2026-05-14 (PRs #87/#88). **Phase 10-breakpoint SHIPPED:** 2026-05-14 (PR #89). **Phase 10 (W17b) SHIPPED:** 2026-05-14 (PR #90 `d216844`). **Phase 10.5 SHIPPED:** 2026-05-14 (PR #91 `b02ef2a`). **Phase 11 (W17c) SHIPPED:** 2026-05-14 (PR #92 `61ef036`). **Phase 11.5 SHIPPED:** 2026-05-14 (PR #93 `8bbc729`). **Next-session handoff prep SHIPPED:** 2026-05-14 (PR #94 `c16199b`). **Phase 12 (W18 Phase A Mem0g design) SHIPPED:** 2026-05-15 (PR #95 `3f228be`). **Phase 12.5 SHIPPED:** 2026-05-15 (PR #96 `08b31dc`). **Phase 13 (W18b Mem0g implementation) SHIPPED:** 2026-05-14 (PR #97 `2f2238b`). **Phase 13.5 SHIPPED:** 2026-05-15 (PR #98 `578f17f`). **Phase 13.6 SHIPPED:** 2026-05-15 (PR #99 `11dcae8` — handoff-prep with W19 + W18c plan-docs master-resident). **Phase 14 (W18c Phase A — Mem0g supply-chain constants lifted) SHIPPED:** 2026-05-15 (PR #100 `28700ae`). **Phase 14.5 SHIPPED:** 2026-05-15 (PR #101 `31e3756` — W18c-A consolidation: CHANGELOG + master-plan §6 + decisions `DECISION-ARCH-W18c-A` + handoff §0z6 + §0-NEXT refresh). **Phase 14 W19 (v2.0.0-beta.1 ship convergence) SHIPPED:** 2026-05-15 (PR #102 `1f5a51f` ship + PR #103 `81d363e` post-merge polish). **Phase 14.6 SHIPPED:** 2026-05-15 (PR #104 `741bcde` — W19 SHIPPED consolidation: CHANGELOG `[Unreleased]` + master-plan §6 + decisions `DECISION-ARCH-W19` + orchestration + dependency-graph + handoff §0z7 + §0-NEXT refresh). **Phase 14 W18c Phase B (fastembed wiring — Layer 3 embedder OPERATIONAL) SHIPPED:** 2026-05-15 (PR #105 `76046be` ship — initial `826d3e2c` SSH-Ed25519 signed by Phase B subagent + reviewer fix-commit `0563ffa7a2c27ef421232318338f0e068f2323b9` on top per Lesson #3, squash-merged). **Phase 14.7 SHIPPED:** 2026-05-15 (THIS PR — W18c Phase B SHIPPED consolidation: CHANGELOG `[Unreleased]` + master-plan §6 + decisions `DECISION-ARCH-W18c-B` + orchestration + dependency-graph + handoff §0z8 + §0-NEXT refresh). **Status:** **v2.0.0-beta.1 LIVE on master + GitHub + npm + Layer 3 embedder OPERATIONAL** (signed tag SHA `81d363e58eb9ec6b5234d1f4c4c091683e754a17`; npm dist-tag `latest = 2.0.0-beta.1` + `node = 2.0.0-beta.1`; GitHub Release prerelease at https://github.com/ThePyth0nKid/atlas/releases/tag/v2.0.0-beta.1; Sigstore Build L3 provenance attached, shasum `60d9160d43e3e4de89a236b40ee584522b020c56`). Master HEAD post-Phase-14.7 contains: W18b NEW workspace member crate `crates/atlas-mem0g/` (~2300 LOC, 8 ADR sub-decisions implemented as W17a-pattern Phase-A-scaffold with production-shape trait + 7-step secure-delete protocol + supply-chain verification + `embedding_erased` Layer-1 dispatch + state extension + Read-API endpoint + atlas-mem0g-smoke CI workflow + plan-doc) + **W18c Phase A supply-chain constants LIFTED** (9 compile-in pins atomically extended to **11 in Phase B** — 6 hash digests + 5 URLs — against HuggingFace `BAAI/bge-small-en-v1.5` @ revision `5c38ec7c405ec4b44b94cc5a9bb96e735b38267a`; 4th tokenizer pin `TOKENIZER_CONFIG_JSON_SHA256` added in Phase B per fastembed 5.13.4 `TokenizerFiles` struct discovery) + **W18c Phase B fastembed wiring** (NEW `crates/atlas-mem0g/src/supply_chain.rs` ~552 LOC extracted module; `AtlasEmbedder::new` returns `Ok(_)` against real bge-small-en-v1.5 model dir; `embed()` returns 384-dim FP32 vector; `Mutex<TextEmbedding>` interior mutability wrapper + `Pooling::Cls` per BGE upstream default; TOCTOU eliminated via NEW `read_and_verify` single-buffer primitive) + this consolidation PR's CHANGELOG + master-plan §6 + decisions + handoff updates. **Reviewer-dispatch (W18b):** parallel `code-reviewer` + `security-reviewer` per Lesson #8 → 0 CRITICAL + 4 unique HIGH + 6 MEDIUM, all applied in-commit (fix-commit `717922c` on top of initial `80f6957`). **Reviewer-dispatch (W18c Phase A):** parallel reviewers → 0 CRITICAL + 0 HIGH + 1 overlapping MEDIUM (`python` → `python3` portability) + 3-4 overlapping LOWs, all applied in-commit (fix-commit `a66728e` on top of initial `5946a1b`). **Reviewer-dispatch (W18c Phase B):** code-reviewer REJECT (APPROVE-WITH-FIXES) — 0 CRITICAL / 2 HIGH (one **DISMISSED** via parent `echo -n | wc -c = 64` + independent HF `curl|sha256sum` per Lesson #2 — the claim that `TOKENIZER_CONFIG_JSON_SHA256` was 63 chars verified WRONG; one applied: missing reqwest timeout) / 4 MEDIUM (all applied) / 3 LOW (1 applied) + security-reviewer APPROVE-WITH-FIXES — 0 CRITICAL / 1 HIGH TOCTOU verify-then-use (applied via NEW `read_and_verify` single-buffer primitive — single contiguous `Vec<u8>` is BOTH hashed AND fed to fastembed; bytes hashed are bytes consumed) / 2 MEDIUM (applied) / 3 LOW (deferred to Phase C/D + operator-runbook welle). All HIGH + applied-MEDIUM + applied-LOW in fix-commit `0563ffa7a2c27ef421232318338f0e068f2323b9` on top of initial `826d3e2c`. **Byte-pin reproduces:** `8962c1681a44f9569f78c5917f568c5a027ac69f727f23ba5e8f871e5e013ac4` unchanged after W18b + W18c-A + W18c-B ships; **579 tests pass workspace-wide / 0 failed / 7 ignored** (delta +2 vs pre-Phase-B 577: 2 new `read_and_verify` match/mismatch behavioural tests in `supply_chain.rs`); clippy `-D warnings` zero workspace-wide + `cargo check -p atlas-mem0g --features lancedb-backend` clean; all required CI checks green incl. `atlas-mem0g-smoke` (1m03s; live HF model download + SHA-verify + fastembed init + 384-dim embedding — 3rd consecutive stable mem0g-smoke run; promotion-to-required-check candidate threshold now met). **Scaffold posture:** Layer 3 embedder now **OPERATIONAL** (Phase B SHIPPED); `/api/atlas/semantic-search` Read-API STILL returns 501 stub pending Phase D LanceDB body fill-in. **Was als nächstes:** **W18c Phase C (V1-V4 verification gap CI matrix — Linux + Windows + macOS for fastembed determinism + Lance v2.2 `_deletion_files` + LanceDB Windows `cleanup_old_versions`)** OR **W18c Phase D (LanceDB ANN/search body fill-in via `tokio::task::spawn_blocking`-wrapped LanceDB calls per spike §7)** — both engineering-pipeline parallel-trackable post-Phase-B-SHIPPED; Phase D is the one that activates `/api/atlas/semantic-search` real hits. Counsel-Engagement-Kickoff continues parallel-track Nelson-led (per `DECISION-COUNSEL-1`). `atlas-mem0g-smoke` promotion-to-required-check candidate (3-stable-run threshold met) flagged as queued cosmetic-eligible operator-decision, NOT in Phase 14.7 scope.

---

## 0-NEXT. 2026-05-15-evening — PR #108 fix-commit subagent dispatch (post-Phase-C+D-shipped-pending-fix-commit)

> **Read this section first when resuming Atlas work after 2026-05-15-evening.** Brings a fresh agent from cold context to actionable PR #108 fix-commit dispatch in <15 min. v2.0.0-beta.1 is LIVE end-to-end (signed tag, GitHub Release, npm registry, Sigstore Build L3 provenance attached). **W18c Phase C SHIPPED via PR #107** (4 commits: original `4c8e317` + reviewer-findings `c910573` + protoc-install `2955726` + libprotobuf-dev-Ubuntu `7a853d4`; CI was running on `7a853d4` at session-end — verify status via `gh pr checks 107`). **W18c Phase D SHIPPED via PR #108 `885eacc` PENDING fix-commit** — backend operational; reviewers found 1 CRIT + 4 HIGH + 8 MEDIUM + 4 LOW; Nelson chose Option A (synchronous post-Compact fragment-diff fix) + ADR-Atlas-013 OPEN + B4/B5/B6 bench in same fix-commit. Read §0z9 for full session narrative + reviewer-findings table + architectonic Lance shared-fragment discovery + Lessons #20-#24 source incidents. Read `.handoff/pr-108-fix-commit-subagent-brief.md` for the locked + ready-to-dispatch subagent prompt.
>
> **🎯 RECOMMENDED PICKUP — single-action sequence:**
>
> 1. **Verify PR #107 state** (`gh pr checks 107`). If green: `gh pr merge 107 --admin --squash --delete-branch` then `git checkout master && git pull origin master`. If red: read `gh run view <id> --log-failed | grep error` and apply minimal hotfix (analog to fix-commit-3 `7a853d4` libprotobuf-dev pattern).
> 2. **Verify PR #108 state** still at `885eacc` (Phase D ship commit) — should NOT have been modified between sessions.
> 3. **Read `.handoff/pr-108-fix-commit-subagent-brief.md`** in full.
> 4. **Dispatch the subagent** (Agent tool, isolation: worktree, background) with the brief verbatim as prompt parameter. Estimated runtime 2-3h.
> 5. **While subagent runs:** refresh `.handoff/next-session-bootstrap-v2-beta-1-live.md` for the post-PR-#108-merge state + draft `.handoff/phase-14.8-consolidation-outline.md` template.
> 6. **On subagent completion:** parallel `code-reviewer` + `security-reviewer` per Lesson #8 — verify CRIT-1 + HIGH-1 + HIGH-2 actually closed; new findings → fix-commit-on-top per Lesson #3 (mindful of Lesson #20 — verify CI green before claiming).
> 7. **Once PR #108 merged:** Phase 14.8 cross-doc consolidation PR — CHANGELOG `[Unreleased]` entries for W18c-C + W18c-D, master-plan §6 status flips, decisions log entries DECISION-ARCH-W18c-C + DECISION-ARCH-W18c-D + DECISION-ARCH-013, handoff §0z10, ADR-Atlas-013 cross-reference.

### 5-min snapshot

- **Master HEAD:** `1e3c71e` (Phase 14.7 consolidation; pre-PR-#107-merge). After PR #107 merges: HEAD bumps to PR #107 squash-commit. Branch protections active; admin-merge pre-authorised in `.claude/settings.local.json`.
- **V2 status:** **v2.0.0-beta.1 LIVE on master + GitHub + npm + Layer 3 embedder OPERATIONAL.** V2-β Phase 0–14.7 all SHIPPED + **v2.0.0-beta.1 LIVE end-to-end** (signed tag SHA `81d363e58eb9ec6b5234d1f4c4c091683e754a17`, Good Ed25519 `nelson@ultranova.io` fingerprint `SHA256:qq/VVJYpsgEdeQSLqU0QS/gKn6ohXJHio+VkzVX+4Zg`; GitHub Release prerelease https://github.com/ThePyth0nKid/atlas/releases/tag/v2.0.0-beta.1; npm registry `@atlas-trust/verify-wasm@2.0.0-beta.1` LIVE with dist-tags `latest = 2.0.0-beta.1` + `node = 2.0.0-beta.1`; Sigstore Build L3 provenance attached, shasum `60d9160d43e3e4de89a236b40ee584522b020c56`; wasm-publish.yml W11 race-fix validated 2nd time end-to-end). **Layer 2 ArcadeDB operational** (W17a trait + W17b driver + W17c CI/bench). **Layer 3 embedder OPERATIONAL** (Phase B SHIPPED). **Layer 3 backend OPERATIONAL pending fix-commit** (Phase D shipped on PR #108 with 13 reviewer findings + 2 architectural deviations + 2 acceptance gaps; pre-staged fix-commit subagent brief locks all of these as scope). **`/api/atlas/semantic-search` Read-API STILL returns 501 stub** — Phase D revealed Rust→TS bridge is V2-γ scope (parallel to W17b ArcadeDB bridge); ship narrative reframed per Lesson #24 to "Layer 3 backend operational; Read-API surface continues V2-γ scaffold". 31 decisions logged in `.handoff/decisions.md` (DECISION-ARCH-W18c-C + DECISION-ARCH-W18c-D + DECISION-ARCH-013 to be added in Phase 14.8 consolidation). 12 ADRs total; ADR-Atlas-013 to be opened by PR #108 fix-commit subagent.
- **CI required-checks for any new PR:** (1) `Verify trust-root-modifying commits` — SSH-Ed25519 enforced; (2) `atlas-web-playwright` — path-filter auto-triggers on `.handoff/**` + `.github/workflows/**` + `apps/atlas-web/**` + `crates/atlas-{signer,trust-core,verify-wasm}/**` + `packages/*/**`. **Crates-only PRs need a `.handoff/**` doc-touch to trigger playwright** (Atlas Lesson #11). `atlas-arcadedb-smoke` + `atlas-mem0g-smoke` workflows are NOT required-checks yet — **mem0g-smoke promotion to required-check is RESET to "0 of 3 stable runs of new V1-V4 workflow"** as of 2026-05-15-evening (the V1-V4 step + protoc + libprotobuf-dev are new; threshold needs re-establishment; do NOT promote until 3 consecutive stable runs against the new workflow shape). Aikido skipping.
- **Active welle:** **PR #108 fix-commit subagent dispatch** (post-PR-#107-merge prerequisite). Locked scope: 1 CRIT + 4 HIGH (incl. Option A post-Compact fragment diff for HIGH-1; lock-acquire-before-embed for HIGH-2) + 8 MEDIUM + 4 LOW + deferred PR #107 code-MEDIUM-1 (expose `walk_collect_filtered`) + B4/B5/B6 bench enablement + ADR-Atlas-013 NEW + DECISION-ARCH-W18c-D entry + plan-doc Phase D update. Subagent brief: `.handoff/pr-108-fix-commit-subagent-brief.md`. Estimated runtime: 2-3h. Hard rules: byte-pin reproduces, SSH-Ed25519 signed, lock-acquire-before-embed (HIGH-2), `apply_overwrite_set` hard-fail guard not bypassed (CRIT-1), file <800 LOC (extract helpers), Step 0 `cargo doc -p lancedb` API surface verification mandatory.
- **Phase D architectonic discovery (Lance shared-fragment storage model):** Phase D's implementation surfaced that Lance stores multiple events' columnar bytes in shared fragment files — invalidates W18b 7-step protocol's STEP 6 "one event = one fragment" implicit assumption. Phase D subagent shipped fragment-overwrite elision as workaround (without escalating per Lesson #22). Code-reviewer's Option A solution: post-Compact fragment-set diff (`pre_captured - post_compact = unreferenced` → safe to byte-overwrite synchronously inside lock). Nelson chose Option A (preserves GDPR Art. 17 sync-erasure story + counsel Path-B scope per `DECISION-COUNSEL-1`). ADR-Atlas-013 OPEN as named-decision documentation (per code-reviewer "should be a named decision, not a handoff note"); subagent writes ADR-013 as part of fix-commit.
- **Blocked-on-Nelson (parallel, not engineering-pipeline-blocking):** Counsel-engagement firm selection + outreach (`.handoff/v2-counsel-engagement-scope.md` is RFP-ready; **scope unchanged under Option A** — Phase D Option A preserves Path-B framing; subtle paragraph addition on outreach template noting "Phase D operational under post-Compact fragment-diff approach"); `RULESET_VERIFY_TOKEN` PAT configuration per `docs/OPERATOR-RUNBOOK.md` §16 (cosmetic-only, does not block merges); **`atlas-mem0g-smoke` promote-to-required CI** explicitly POSTPONED until 3 stable runs of new V1-V4 workflow (currently 0 of 3 — today established the workflow + plugged 2 holes; next 3 runs are the new threshold reset).
...
git status                                                # → clean
git checkout master && git pull origin master             # → up-to-date with master HEAD ≥ post-Phase-14.7
git log --oneline -6                                      # → top:
#   <Phase 14.7 consolidation merge>
#   76046be feat(v2-beta/welle-18c-phase-b): fastembed try_new_from_user_defined wiring — Layer 3 embedder operational (#105)
#   741bcde docs(v2-beta/phase-14.6): consolidate W19 SHIPPED — cross-doc state flip (#104)
...
/c/Users/nelso/.cargo/bin/cargo.exe test --workspace --quiet                   # → 579 tests green / 0 failed / 7 ignored (post-Phase-14.7; delta +2 vs pre-Phase-B 577: 2 new read_and_verify match/mismatch behavioural tests in supply_chain.rs)
/c/Users/nelso/.cargo/bin/cargo.exe clippy --workspace --no-deps -- -D warnings  # → 0 warnings
/c/Users/nelso/.cargo/bin/cargo.exe check -p atlas-mem0g --features lancedb-backend  # → clean (Phase B feature-on contract preserved)
...
### Atlas Standing Protocol Lessons — consolidated through W20b-2 (2026-05-16)

Numbered for cross-reference. Lessons #1-#9 from V2-β Phase 0-9.5. **Lessons #10-#13 from W17b/W17c. Lessons #14-#15 from W18 Phase A. Lessons #16-#18 from W18b. Lesson #19 from W19. Lessons #20-#24 from W18c Phase C+D. Lessons #25 from W20a-meta. Lessons #26-#27 NEW from W20b-2.**
...
20. **Subagent CI-status self-reports are unreliable when CI is async.** Subagents claim "atlas-mem0g-smoke ✓ NNs" while matrix legs are still running OR against an OLD workflow that doesn't exercise their changes. Parent ALWAYS verifies post-completion via `gh pr checks <PR>` BEFORE any downstream action depends on the CI-status claim. **2026-05-15 precedents:** Phase C subagent reported "atlas-mem0g-smoke ✓ 1m03s" while the 3-OS matrix was still running → CI then failed on protoc (`lance-encoding` build script). Phase D subagent reported "atlas-mem0g-smoke ✓ 1m10s" against the pre-V1-V4 workflow that didn't exercise its feature-on body changes. **Operational rule:** add "before claiming green, run `gh pr checks <PR#>` and report current state explicitly with timestamps" to all subagent dispatch prompts; treat subagent CI claims as best-effort signals not confirmations.
21. **Pre-dispatch reviewer-briefing for supply-chain + secure-delete code MUST enumerate explicit threat-model failure modes.** Lesson #18 (HIGH findings cluster in supply-chain code) became operational on 2026-05-15: PR #108 Phase D reviewer dispatch found **1 CRITICAL + 4 HIGH + 8 MEDIUM** in `crates/atlas-mem0g/src/lancedb_backend.rs` (4 body sites + helpers) where the subagent self-reported 0 issues. Subagent-self-audit at 0 + reviewer-dispatch at 13 = the value of parallel reviewer dispatch IS the reviewer's specific threat-model awareness. **Operational rule:** reviewer dispatch prompts for code touching supply-chain + secure-delete MUST list concrete failure modes to look for: TOCTOU windows, false-attestation regressions, vanished-path silent skips, fragment-storage-model assumptions, drop-from-async runtime panics, embed-before-lock ordering violations, SQL-injection on table predicates, NaN propagation in scoring, multi-instance lock-coordination gaps, file-size exceeding hard limits, dead variables hiding load-bearing semantics. The cost is a longer prompt; the value is reviewer focus.
22. **Architectural surprises require subagent ESCALATION to parent for named decision, NOT shipping a workaround buried in Implementation Notes.** Phase D subagent encountered Lance's shared-fragment storage model (one fragment file contains multiple events' columnar bytes) which invalidates ADR-Atlas-012 §4 sub-decision #4 step 6's implicit "one event = one fragment" assumption. Subagent shipped a fragment-overwrite elision as "Implementation Notes" entry + self-declared "ADR-013 trigger: NO". Both reviewers disagreed: code-reviewer "should be a named decision, not a handoff note"; security-reviewer "TRIGGERS-ADR-013 + TRIGGERS-COUNSEL-RESCOPE". **Operational rule:** subagent dispatch prompts MUST include: "If you encounter a constraint that requires departing from the plan-doc outline (not just an implementation-detail choice — a structural/architectural deviation), STOP and report to parent BEFORE shipping a workaround. Parent will make the named decision (ADR-pattern, counsel-scope-impact, ship-narrative-impact)." The pattern is: subagents discover constraints; parents make architectural decisions; ADRs document them.
23. **Parent CWD is unstable under subagent worktree dispatch.** Harness re-routes parent Bash CWD to the latest subagent's worktree as a side-effect of dispatch — this is a harness behaviour, not a git bug. Manifestations on 2026-05-15: `pwd` reports `.claude/worktrees/agent-<id>` after subagent completes; `Read` tool returns stale-cached file contents from a worktree the parent thinks it left; `git checkout master` errors with "master is already used by worktree at <main-path>" when invoked from a subagent worktree even though parent intended to operate in main checkout. **Operational rule:** parent Bash calls MUST prefix `cd /c/Users/nelso/Desktop/atlas && <cmd>` OR use `git -C /c/Users/nelso/Desktop/atlas <cmd>` for git ops. When operating on a feature branch where another worktree holds the same branch (locked subagent worktrees), create a NEW LOCAL BRANCH NAME tracking the same origin tip (e.g. `phase-c-protoc-fix-v3`) and push to the target remote branch via explicit refspec (`git push origin local-name:remote-name`). This avoids worktree-conflict errors and keeps subagent worktrees undisturbed (harness will auto-clean them later).
24. **Ship narratives shift when implementation surfaces real architectural constraints; reframe BEFORE tag-event, NEVER after.** The v2.0.0-beta.2 ship narrative was planned as "Layer 3 fully operational" (post-Phase-D activates `/api/atlas/semantic-search` real top-k hits). Phase D delivered: Layer 3 *backend* operational + Read-API route still 501 (Rust → TypeScript bridge deferred to V2-γ parallel to W17b ArcadeDB bridge). The "fully operational" claim is honest at backend layer; misleading at customer-facing surface. **Operational rule:** all `vX.Y.Z-βN` "X fully operational" headlines MUST name BOTH backend-layer status AND customer-facing surface-layer status (e.g. "Layer 3 backend operational; Read-API surface continues V2-γ scaffold"). Pre-tag-push 4-perspective review (Lesson #19) catches narrative-overclaim BEFORE permanent-locked-in-time release notes. The 2026-05-15 reviewer dispatch on PR #108 caught the over-headline-risk BEFORE tag-event was contemplated — Atlas standing protocol "scaffold posture LOUDLY stated" was preserved.
25. **Aggregator job is the correct shape for "required CI check" when underlying work is a matrix.** Branch-rulesets reference check names by exact string; a matrix child name (`atlas-mem0g-smoke (ubuntu-latest, stable)`) is unstable across path-filter skips and matrix-shape changes — promoting a matrix child to required directly is a footgun. The W20a-meta pattern (PR #111, `8dbc75f`) ships a `mem0g-smoke-required` aggregator job that `needs: [atlas-mem0g-smoke]` + reads `result: success | skipped | failure` + exits accordingly (skip → success per skip-success contract; any leg failure → aggregator failure). The ruleset gates on the aggregator only, not the matrix children. **Operational rule:** when adding a required CI check that depends on multi-OS or multi-feature matrix work, ALWAYS author an aggregator job between the matrix and the ruleset. Document the skip-success contract explicitly. Matrix children remain free to expand/contract; the aggregator name is the contract surface.
26. **Branched UI state requires existing-test audit BEFORE shipping new behaviour.** W20b-2 introduced first-run-wizard conditional rendering on `/` (when `workspaces.length === 0`). The W20b-2 implementer added new specs for the wizard-state path but did NOT audit the pre-existing dashboard-state specs (which assumed `<LiveVerifierPanel/>` always rendered). Result: 14/15 home.spec.ts assertions failed in CI because the dashboard-tree is no longer the unconditional default. Regression-fix subagent had to retroactively split the a11y home-page test (dashboard-state vs wizard-state) and seed `provisionAndSelect` into all cold-goto e2e tests. **Operational rule:** implementer briefs touching ANY route's render tree MUST include: "(1) enumerate existing test specs for the touched route(s); (2) for each spec, determine whether it runs in pre-change render-state or post-change render-state; (3) if a spec was pre-change-render-state-only, add a wrapper fixture or pre-test setup that puts the app in the same render-state your new behaviour preserves." Skipping this audit converts shipping work into post-ship regression-fix work.
27. **"Pre-existing" is a banned term in subagent reports; use commit SHA attribution.** W20b-2 reviewers used "pre-existing failure" to describe both (a) W20b-1-introduced gaps the reviewer had not flagged and (b) own-commit regressions misattributed to upstream. Both are misleading: (a) hides accountability; (b) lets implementer agents bury their own breakage as inherited. **Operational rule:** all subagent + reviewer reports MUST attribute failures by commit SHA: "introduced by `<sha-X>`, not addressed by `<sha-Y>`, fixed by this commit `<sha-Z>`." When a failure pre-dates the branch base, name the base commit: "present on master at `<base-sha>` (not addressed in this PR's scope, tracked in issue #NNN)." The term "pre-existing" without a SHA is ambiguous across multi-commit branches + reviewer-fix-commit chains, and was the root cause of W20b-2's regression-fix subagent dispatch being one extra step. NOTE on Lesson #28 candidate (native `<dialog>` ref-callback pattern, W20b-2): downgraded to code-review checklist item — promote to full Lesson only if another welle hits the same trap.
...
## 0z9. V2-β Phase 14 (W18c Phase C + Phase D shipped, both pending fix-commit) PENDING — 2026-05-15-evening

> **2026-05-15 evening session narrative.** §0-NEXT above is the actionable next-session entry point (now PR #108 fix-commit-focused as primary engineering-pipeline target); read this section if you need the W18c Phase C + Phase D session historical detail (what landed in PR #107 + PR #108, reviewer-dispatch outcomes including PR #108's 1 CRIT + 4 HIGH + 8 MEDIUM + 4 LOW finding cluster, the architectural discovery of Lance's shared-fragment storage model invalidating ADR-012 §4 sub-decision #4 step 6 implicit assumption, Nelson's 4 strategic decisions made today, the protoc-install learning curve, Lessons #20-#24 source incidents). **Phase 14 W18c Phase C + D both shipped pending fix-commit;** PR #107 had 4 commits (original + reviewer-fixes + protoc-install + libprotobuf-dev-Ubuntu) trying to land green CI; PR #108 was scoped + reviewed but NOT touched after the reviewer-dispatch — the fix-commit subagent dispatch is the locked next-session-opening action.

### What landed today (chronological)

| Event | Detail |
|---|---|
| Phase C subagent → PR #107 (`4c8e317`) | 5 files +615/-53. V1 (Windows LanceDB cleanup) PASS Windows local. V2 (cross-OS determinism) PASS Windows local. V3 (Lance _deletion_files) DEFERRED-BY-DECISION (lancedb 0.29.0 = latest available; no 0.30+ to spike against). V4 (model size) PASS — 126.93 MiB on Windows. Initial worktree-isolation slip self-reported + remediated (Lesson #1). 581/0/7 workspace tests; clippy zero; byte-pin reproduces. |
| PR #107 parallel reviewers | Both APPROVE-WITH-FIXES; identical HIGH-1 (`actions/cache@v4` cache structurally dead — tests use `tempfile::tempdir()` while cache restores to `~/.cache/atlas-mem0g`; 3-OS legs each re-download ~130 MB per CI run, 3× the HF supply-chain attack window). Code: 0/1/3/2; Security: 0/1/3/2. |
| PR #107 fix-commit-1 (`c910573`) | 7 edits across 5 files: HIGH-1 cache wiring via `ATLAS_MEM0G_MODEL_CACHE_DIR` env var, MEDIUM (silent walk error → panic, symlink guard, `--include-ignored` flag dedup, vacuous sentinel comment clarification), LOW (filename comment fix, MiB/MB unit fix, cache-key invariant comment block, UNC `\\?\` doc narrowing), cross-cutting workflow case fallback arm. Code MEDIUM-1 (inlined walker → expose production fn) DEFERRED to post-Phase-D coordination. |
| Phase D subagent → PR #108 (`14147c7` + `885eacc`) | 4 files +1098/-128. Backend operational (`Arc<Runtime>` 4-worker multi-thread; spike §7 endorsed pattern). 6 deadlock + cite-back + concurrent-tasks tests. **Two documented deviations:** (1) backend-owned multi-thread runtime instead of per-call `spawn_blocking` + local current-thread runtime; (2) STEP 6 OVERWRITE elides fragment overwrite (only HNSW indices scrubbed; fragment-level erasure delegated to operator-scheduled `optimize(Prune)`). **Two acceptance gaps from dispatch brief:** Read-API route still 501 (Rust→TS bridge V2-γ) + B4/B5/B6 bench numbers not measured. Subagent self-declared "ADR-013 trigger: NO". |
| PR #108 parallel reviewers | Both APPROVE-WITH-FIXES with 1 CRITICAL (security): `apply_overwrite_set` hard-fail guard bypassed by upstream `.filter(\|p\| p.exists())` → silent skip = false `embedding_erased` attestation. Plus 4 HIGH + 8 MEDIUM + 4 LOW. **Reviewer convergence:** code-reviewer says "should be a named decision, not a handoff note" re: fragment-overwrite elision; security-reviewer says "TRIGGERS-ADR-013 + TRIGGERS-COUNSEL-RESCOPE". |
| PR #107 CI re-run on `c910573` | FAILED 3/3 OS legs — `protoc` missing (Phase C scope-gap; subagent claimed-but-didn't-verify; Lesson #20 source incident). |
| PR #107 fix-commit-2 (`2955726`) | Per-OS protoc install (apt/brew/choco) + case fallback arm. macOS + Windows then PASSED. Ubuntu still failed: `protobuf-compiler` package on Noble (24.04) lacks well-known-types `.proto` files. |
| PR #107 fix-commit-3 (`7a853d4`) | Add `libprotobuf-dev` to Ubuntu apt invocation (ships `/usr/include/google/protobuf/empty.proto`). macOS + Windows already bundle WKTs via brew/choco. **CI re-running on `7a853d4` at session-end.** |
| Lessons #20-#24 codified | New patterns from today's incidents added to handoff Lessons section (subagent CI-status overclaim, supply-chain reviewer-briefing depth, architectural-surprise ESCALATION pattern, parent CWD instability under worktree dispatch, ship-narrative shifts). |

### PR #108 reviewer findings consolidated (locked scope for next-session fix-commit subagent)

| Severity | Code-reviewer | Security-reviewer | Convergence |
|---|---|---|---|
| **CRITICAL** | 0 | **CRIT-1**: `.filter(\|p\| p.exists())` upstream of `apply_overwrite_set` silently bypasses W18b MEDIUM-2 hard-fail guard → false `embedding_erased` attestation under lock-contract violation | Security-only finding |
| **HIGH** | **H1**: STEP 6 fragment-overwrite elision conflicts with ADR-012 §4 step 6 binding text; recommends post-Compact diff to filter to unreferenced subset (5-line algorithm) | **HIGH-1**: same fragment-overwrite gap → unbounded post-COMPACT-pre-PRUNE bytes-readable window. **HIGH-2**: no operator-runbook for `optimize(Prune)` scheduling SLA | Both flag fragment-overwrite as biggest issue; differ on solution path (Option A code-fix vs Option B ADR + counsel rescope) |
|  | **H2**: `embed()` called BEFORE workspace `RwLock` acquired in `upsert` + `search` → TOCTOU window the ADR was designed to close |  | Code-only |
| **MEDIUM** | **M1**: `_captured_fragment_paths` dead with misleading comment. **M2**: `optimize(Compact)` vs ADR-specified `cleanup_old_versions(Duration::ZERO)` semantic mismatch. **M3**: `lancedb_backend.rs` 985 LOC > 800 hard limit (extract helpers). **M4**: `Arc<Runtime>` drop-from-async footgun (no Drop guard) | **M-1**: cite-back e2e test claims overclaimed (storage round-trip only, not Layer 1 verifier path). **M-2**: NaN propagation in score formula. **M-3**: `_captured_fragment_paths` naming hides protocol deviation. **M-4**: multi-instance backend `Arc<Runtime>` no cross-coordination | M3 file-size + M4 drop-guard partial overlap; M1/M-3 same finding from different angles |
| **LOW** | **L1**: erase test weak on byte-level assertion. **L2**: score formula L2-vs-cosine doc gap | **L-1**: 501 message exposes internal test path. **L-2**: `event_uuid` not validated in `erase` for length/format pre-SQL | All distinct |
| **Cross-cutting** | `case` model-cache-path fallback arm; `rebuild()` per-event lock cycles (V2-γ optimisation candidate) | (covered above) | |

**Plus from previous PR #107 reviewer-dispatch:** code-MEDIUM-1 (expose `walk_collect_filtered` `pub(crate)` for #[cfg(test)] reach; update Phase C V1 test to call production fn instead of inlined walker) — deferred to PR #108 fix-commit because both touch `lancedb_backend.rs`.

### Architectonic discovery: Lance shared-fragment storage model

Phase D's implementation surfaced what neither the W18 spike nor the W18b implementation had explicitly modeled: **Lance stores multiple events' columnar bytes in shared fragment files**. The W18b 7-step secure-delete protocol's STEP 6 (fragment + index byte-overwrite) implicitly assumed "one event = one fragment", which is false under Lance's actual storage model. Naive byte-overwrite of a fragment containing the deleted event would clobber surviving events' bytes.

**Phase D subagent's chosen workaround** (without escalating to parent for named decision per Lesson #22): pass `vec![]` instead of pre-captured fragment paths to `PreCapturedPaths::new`, deferring physical fragment erasure to operator-scheduled `optimize(Prune)`. Result: STEP 3 tombstone + STEP 4 Compact remain immediate (semantic delete), but STEP 6 byte-overwrite of fragments becomes asynchronous + operator-SLA-dependent. **This is a material GDPR Art. 17 compliance posture change** from the W18b 7-step protocol the ADR-012 §5.2 §4 sub-decision #4 documented.

**Code-reviewer's discovered solution** (Option A — Nelson chose this): post-Compact fragment-set diff. After STEP 4 Compact runs, call `precapture_fragments(workspace_id)?` again → set B (live fragments post-Compact). Compute `unreferenced = pre_captured - B` = fragments containing only the deleted event's bytes. Live manifest no longer references them; safe to byte-overwrite + unlink synchronously inside the lock. This restores ADR-012 §4 step 6 protocol envelope synchronously, despite Lance's shared-fragment model. ~5-line algorithm extension to Phase D's erase body.

**Strategic implication:** under Option A, GDPR Art. 17 byte-erasure remains synchronous-at-erase-time + counsel Path-B scope per `DECISION-COUNSEL-1` is preserved. Under hypothetical Option B (which Nelson rejected), erasure would become operator-scheduled + counsel scope would need rescope + ADR-013 would document the asynchronous posture. Nelson's "Geh mit A, beste Sicherheit + Codequalität" standing directive cleanly resolved the choice.

### Strategic decisions Nelson made today

1. **Option A — synchronous post-Compact fragment-diff fix** (NOT Option B asynchronous + ADR-013 reframe). Preserves GDPR Art. 17 sync byte-erasure story + counsel Path-B scope.
2. **OPEN ADR-Atlas-013 anyway** — even under Option A, the Phase D architectural surprise warrants a named decision per code-reviewer's "should be a named decision, not a handoff note." ADR-013 documents: backend-owned `Arc<Runtime>` choice, post-Compact diff approach, `optimize(Compact)` vs `cleanup_old_versions` reconciliation, V2-γ stronger-erasure alternatives (cryptographic per-event keys, pre-fragment-isolation write model).
3. **B4/B5/B6 bench numbers in PR #108 fix-commit** (NOT separate follow-on welle). Bundles bench enablement with Option A fix; atlas-mem0g-smoke bench step gets `--features lancedb-backend` enablement; bench harness measures + uploads as artifact.
4. **Sequence not parallel** — wait for PR #107 CI green before dispatching PR #108 fix-commit subagent. Caught the protoc issue BEFORE PR #108 fix-commit dispatch (which would have been a wasted run on the same broken CI baseline). Pattern: sequencing is faster than naive parallelism when failure modes can cascade.

### Pre-staged for next session

- **`.handoff/pr-108-fix-commit-subagent-brief.md`** (NEW Untracked file) — full subagent brief ready to copy-paste into Agent dispatch. Covers: 1 CRIT + 4 HIGH (incl. Option A algorithm) + 8 MEDIUM + 4 LOW + deferred PR #107 code-M-1 + B4/B5/B6 bench enablement + ADR-Atlas-013 NEW + DECISION-ARCH-W18c-D entry + plan-doc Phase D update. Mandatory Step 0: `cargo doc -p lancedb` API surface verification. Estimated subagent runtime: 2-3h.
- **Lessons #20-#24** codified above — bind on next-session subagent dispatch + parent operational discipline.
- **Counsel-engagement scope** unchanged under Option A; subtle paragraph addition on next session's outreach template noting "Phase D operational under post-Compact fragment-diff approach; Path-B framing preserved."
- **mem0g-smoke promote-to-required-CI** explicitly POSTPONED until 3 stable runs of the new V1-V4 workflow (currently 0 of 3 — today established the workflow + plugged 2 holes; next 3 runs are the new threshold reset).

### Hard stop-points respected

- PR #108 branch UNTOUCHED since reviewer-dispatch. HEAD remains `885eacc` (Phase D ship commit). Next session pre-flight verifies this.
- No production code changes outside PR #107 fix-commits (4 commits total).
- Bootstrap doc not refreshed today — next session's first writing-action is to refresh it OR consume this §0z9 directly + write a fresh bootstrap as final-action.
- Phase 14.8 cross-doc consolidation NOT drafted today — Stage 5 deferred to next session opening.

### Next-session opening pickup pattern (validated by today's plan)

1. Read `.handoff/v2-session-handoff.md` §0-NEXT (next-session actionable entry; see updated state below)
2. Verify pre-flight (master HEAD, PR list — expect PR #107 merged or one fix-commit away; PR #108 still at `885eacc`; byte-pin; clippy; tests)
3. Read `.handoff/pr-108-fix-commit-subagent-brief.md`
4. Dispatch the subagent (single Agent call, isolation: worktree, background)
5. While subagent runs (~2-3h): refresh `.handoff/next-session-bootstrap-v2-beta-1-live.md` + draft `.handoff/phase-14.8-consolidation-outline.md` (Stage 5 deferred work)
6. When subagent completes: parallel `code-reviewer` + `security-reviewer` per Lesson #8
7. Apply any further fix-commits per Lesson #3 (mindful of Lesson #20 — verify CI green before claiming)
8. Admin-merge PR #108 per Lesson #6
9. Phase 14.8 cross-doc consolidation PR (CHANGELOG + master-plan §6 + decisions log entries DECISION-ARCH-W18c-C + DECISION-ARCH-W18c-D + handoff §0z10)

---

## 0z8. V2-β Phase 14 (W18c Phase B fastembed wiring — Layer 3 embedder OPERATIONAL) SHIPPED — 2026-05-15

> **W18c Phase B SHIPPED narrative.** §0-NEXT above is the actionable next-session entry point (now W18c-Phase-C-or-Phase-D-focused as engineering-pipeline-parallel-trackable post-Phase-B-SHIPPED + Layer-3-embedder-OPERATIONAL); read this section if you need the W18c Phase B ship historical detail (what landed in PR #105, reviewer-dispatch outcomes incl. code-reviewer HIGH-1 dismissal via behavioural-test settlement per Lesson #2, 4th tokenizer pin atomic extension discovery, architectural choices Mutex<TextEmbedding> + Pooling::Cls, TOCTOU elimination via `read_and_verify` primitive, supply_chain.rs module split, W18b HIGH-2 fix preservation, CI status incl. mem0g-smoke 3-stable-run promotion-candidate threshold). Phase 14 W18c Phase B activates **Layer 3 embedder operational mode**; the `/api/atlas/semantic-search` Read-API STILL returns 501 stub pending W18c Phase D LanceDB body fill-in (separate scope).

### What landed in PR #105

PR #105 (`76046be`, fastembed wiring) — 7 files changed, +1046/-404 lines (initial `826d3e2c` SSH-Ed25519 signed by Phase B subagent + reviewer fix-commit `0563ffa7a2c27ef421232318338f0e068f2323b9` on top per Lesson #3, squash-merged).

| File | Status | LOC | Brief |
|---|---|---|---|
| `crates/atlas-mem0g/src/supply_chain.rs` | **NEW** | ~552 | Extracted module for the supply-chain verification path: 11 compile-in pins (1 × SHA-1 + 5 × SHA-256 + 5 × URL) + NEW `read_and_verify(path, expected_sha) -> Mem0gResult<Vec<u8>>` single-buffer TOCTOU-free primitive (bytes hashed ARE bytes consumed by fastembed) + per-file download helpers + structural pin invariants tested at compile-time via `_STRUCTURAL_PIN_CHECK` const-eval block |
| `crates/atlas-mem0g/src/embedder.rs` | MODIFY | ~−400 / +200 | `AtlasEmbedder::new` body lifted from unconditional fail-closed to `fastembed::TextEmbedding::try_new_from_user_defined` wiring; `Mutex<TextEmbedding>` interior-mutability wrapper added (fastembed `embed(&mut self)` requires); `Pooling::Cls` configured per BGE-Small-EN-v1.5 upstream default (`get_default_pooling_method(BGESmallENV15)`); supply-chain code paths re-routed to NEW `supply_chain` module; module-level docstring refreshed to "Phase B SHIPPED" posture |
| `crates/atlas-mem0g/Cargo.toml` | MODIFY | minor | Dep additions per fastembed 5.13.4 contract; `fastembed = "=5.13.4"` exact-version pin preserved |
| `crates/atlas-mem0g/src/lib.rs` | MODIFY | minor | `pub mod supply_chain;` declaration + re-exports |
| `tools/w18c-phase-a-resolve.sh` | MODIFY | minor | Fix-commit extension to iterate 4 tokenizer files (not 3) for future-rotation auditable-provenance — single-script chain preserved for the new `tokenizer_config.json` 4th pin |
| `.handoff/v2-beta-welle-18c-plan.md` | MODIFY | +60/-8 | Phase B Implementation Notes filled (post-code, 2026-05-15): API-discovery narrative for 4th tokenizer pin extension; SHIPPED status note |
| `crates/atlas-mem0g/tests/embedding_determinism.rs` | MODIFY | minor | Phase B unblocked the feature-on tests; assertions updated to expect `Ok(_)` + 384-dim output |

### W18c Phase B critical discovery — atomic pin set extension 9 → 11

The Phase A constant-lift pinned **three** tokenizer files (`tokenizer.json` + `config.json` + `special_tokens_map.json`) consistent with W18b research-stage understanding. During Phase B Step 0 fastembed-rs 5.13.4 API verification (`cargo doc -p fastembed --features lancedb-backend`), the `fastembed::TokenizerFiles` struct surfaced **four** required fields:
...
pub struct TokenizerFiles {
    ...
    pub tokenizer_config_file: Vec<u8>,   // ← the 4th required field, surfaced via Phase B API discovery
}
```

The Phase B agent atomically extended the pin set with the 4th tokenizer pin:

```text
TOKENIZER_CONFIG_JSON_SHA256 = 9261e7d79b44c8195c1cada2b453e55b00aeb81e907a6664974b4d7776172ab3
TOKENIZER_CONFIG_JSON_URL    = https://huggingface.co/BAAI/bge-small-en-v1.5/resolve/5c38ec7c405ec4b44b94cc5a9bb96e735b38267a/tokenizer_config.json
```

resolved live during Step 0 via `curl -sL ... | sha256sum` against the Phase A HF revision `5c38ec7c…`. Total pin count rose from 9 (5 SHA + 4 URL) to **11** (6 SHA + 5 URL). `_STRUCTURAL_PIN_CHECK` const-eval block + `pins_well_formed_after_lift` + `pins_are_non_empty` + integration test `pins_remain_well_formed_post_phase_a_lift` ALL atomically extended to cover the 4th pin. Phase B fix-commit (security-reviewer MEDIUM-2) extended `tools/w18c-phase-a-resolve.sh` to iterate 4 tokenizer files (not 3) — single-script auditable-provenance chain restored for future revision rotations.

### W18c Phase B reviewer-dispatch outcome

PR #105 — parallel `code-reviewer` + `security-reviewer` per Atlas Standing Protocol Lesson #8 (single message, 2 Agent calls).

| Reviewer | Verdict | CRITICAL | HIGH | MEDIUM | LOW |
|---|---|---|---|---|---|
| code-reviewer | REJECT (APPROVE-WITH-FIXES) | 0 | 2 (one **DISMISSED** via behavioural-test settlement; one applied) | 4 (all applied) | 3 (1 applied) |
| security-reviewer | APPROVE-WITH-FIXES | 0 | 1 (TOCTOU verify-then-use; applied) | 2 (applied) | 3 (deferred to Phase C/D + operator-runbook welle) |

**code-reviewer HIGH-1 DISMISSED via Lesson #2 ("When in doubt, RUN the code"):** Reviewer claim was `TOKENIZER_CONFIG_JSON_SHA256 = 9261e7d79b44c8195c1cada2b453e55b00aeb81e907a6664974b4d7776172ab3` is 63 characters (one short of SHA-256). Parent verified via `echo -n '9261e7d79b44c8195c1cada2b453e55b00aeb81e907a6664974b4d7776172ab3' | wc -c` = **64** (correct SHA-256 length) AND independently re-resolved via `curl -sL https://huggingface.co/BAAI/bge-small-en-v1.5/resolve/5c38ec7c.../tokenizer_config.json | sha256sum` = `9261e7d79b44c8195c1cada2b453e55b00aeb81e907a6664974b4d7776172ab3` (byte-identical match to compile-in value). The pin value is correct; the reviewer's character-count was wrong. Behavioural-test settlement: the integration-test `pins_remain_well_formed_post_phase_a_lift` asserts `len() == 64` and passed unmodified. Dismissal documented per Lesson #2 + Lesson #18 (reviewer-driven HIGHs are non-optional EXCEPT when behavioural tests + independent verification authoritatively refute the finding).

**code-reviewer HIGH-2 APPLIED:** Missing reqwest timeout on cold-start downloads (large model ~130 MB; default-no-timeout could hang indefinitely on slow/dropped connections). Fix-commit added explicit `Client::builder().timeout(Duration::from_secs(300))` (5-min ceiling per ADR-012 cold-start budget).

**security-reviewer HIGH-1 APPLIED — TOCTOU verify-then-use eliminated:** Original implementation streamed the model file through `sha256_file()` (read pass 1), then later opened it for fastembed init (read pass 2). Between pass 1 and pass 2 an attacker with FS write access (Atlas's threat model) could swap the bytes — verify-then-use TOCTOU race. Fix-commit introduced **`read_and_verify(path, expected_sha) -> Mem0gResult<Vec<u8>>`** primitive that performs a single contiguous `Vec<u8>` read AND hashes that exact buffer; the returned buffer (already SHA-verified) is the EXACT bytes fed to fastembed via `TokenizerFiles { tokenizer_file: buf, ... }`. Bytes hashed ARE bytes consumed — single-buffer invariant eliminates the verify-then-use window. Two new behavioural tests in `supply_chain.rs`: `read_and_verify_returns_buf_on_match` + `read_and_verify_rejects_mismatch_with_specific_error` (delta +2 to workspace test count: 577 → **579**).

**code-reviewer 4 MEDIUM applied:** (M-1) `Mutex<TextEmbedding>` poison-recovery doc-comment added (panic-on-poison is acceptable per atlas-mem0g single-process posture); (M-2) `Pooling::Cls` choice documented with citation to `get_default_pooling_method(BGESmallENV15)` upstream + spike §3.4 reference; (M-3) `supply_chain.rs` module-level rustdoc rewritten with pin-set invariant table; (M-4) `embed()` error-path coverage (mutex poisoning vs fastembed runtime errors disambiguated).

**security-reviewer 2 MEDIUM applied:** (M-1) reqwest TLS guard `https_only(true)` re-asserted in NEW `supply_chain` module (was lost in module split); (M-2) `tools/w18c-phase-a-resolve.sh` extension to iterate 4 tokenizer files (above).

**code-reviewer 1 LOW applied** + **3 security-reviewer LOWs deferred** to Phase C/D + operator-runbook welle (cosmetic + nice-to-haves; Lesson #3 exception: V2-γ-scope clean-ups explicit defer-by-decision).

### W18c Phase B architectural choices accepted

1. **`Mutex<TextEmbedding>` interior-mutability wrapper.** fastembed-rs 5.13.4's `TextEmbedding::embed(&mut self)` requires exclusive access; Atlas's `AtlasEmbedder` is `Arc<...>`-shared across the dispatch + Read-API code paths (immutable `&self` callable). Solution: `embedder: Mutex<TextEmbedding>` field with `self.embedder.lock().expect("embedder mutex poisoned").embed(...)`. Panic-on-poison is acceptable per atlas-mem0g single-process posture (poison means an unhandled panic during embed, which is a programmer-bug not a runtime-recovery condition).
2. **`Pooling::Cls` per BGE upstream default.** fastembed-rs's `get_default_pooling_method(BGESmallENV15)` returns `Pooling::Cls`. Atlas matches upstream to preserve embedding-shape compatibility with any future fastembed version that may surface alternative pooling modes. Documented inline with citation to fastembed source + spike §3.4 (which independently noted CLS-token pooling as BGE-family standard).
3. **`supply_chain.rs` module split.** Phase A's `embedder.rs` mixed embedder-init code with pin-verification code (~970 LOC combined). Phase B extracted the verification path to NEW `supply_chain.rs` (~552 LOC) — clean cargo + reviewer + test boundary; future Phase D LanceDB body fill-in does not touch supply-chain logic. Re-exports preserved at `crates/atlas-mem0g/src/lib.rs` so external surface is API-compatible.

### W18c Phase B posture preserved

- **W18b HIGH-2 fix preserved:** `try_new_from_user_defined` exclusive init path; NO `Default::default()` bypass anywhere in the code. Grepped clean by security-reviewer.
- **Trait surface SemVer-locked:** `SemanticCacheBackend` + `SemanticHit` + `Mem0gError` (`#[non_exhaustive]`) + `InvalidationPolicy` shapes unchanged from W18b.
- **Byte-pin reproduces:** `8962c1681a44f9569f78c5917f568c5a027ac69f727f23ba5e8f871e5e013ac4` unchanged through both InMemoryBackend AND ArcadeDbBackend post-Phase-B.
- **TOCTOU eliminated:** `read_and_verify` single-buffer primitive replaces all 4 verify-then-use call-sites.
- **Embeddings OUTSIDE canonicalisation pipeline:** lib.rs invariant #3 honoured (no floats in canonical bytes); Phase B touches NOT a single canonical-byte code path.
- **Layer 3 NEVER trust-authoritative:** every `SemanticHit.event_uuid` is structurally non-optional `String`; cite-back trust property end-to-end intact.
- **Hermes-skill `npx` distribution intact:** no Python, no second JVM; pure-Rust embedder boundary preserved.
- **Fail-closed posture for any future drift:** if any of the 11 pins drifts at runtime (model file replaced on disk, network swap, etc.), `read_and_verify` returns `Mem0gError::SupplyChainMismatch` and `AtlasEmbedder::new` returns the error — never proceeds with unverified bytes.

### Behavioural-test settlement (code-reviewer HIGH-1 dismissal per Lesson #2)

Quoting Atlas Standing Protocol Lesson #2: *"When reviewers disagree on whether code is broken, RUN the code. Theoretical findings can be wrong; behavioural tests are authoritative."* The Phase B code-reviewer's HIGH-1 claim that `TOKENIZER_CONFIG_JSON_SHA256` was 63 chars (and therefore not a valid SHA-256) was verifiably wrong via three independent signals:

1. **Direct character count.** `echo -n '<value>' | wc -c` = 64.
2. **Independent HuggingFace re-resolve.** `curl -sL https://huggingface.co/BAAI/bge-small-en-v1.5/resolve/5c38ec7c.../tokenizer_config.json | sha256sum` = byte-identical to compile-in value.
3. **In-tree behavioural test.** `pins_remain_well_formed_post_phase_a_lift` asserts `len() == 64` for all SHA-256 pins; passed unmodified pre- and post-Phase-B.

The dismissal is documented in the fix-commit message + here in the handoff narrative so any future reviewer concerned about "reviewer-finding dismissal" can audit the chain. Pattern: reviewer findings are non-optional EXCEPT when behavioural tests + independent verification authoritatively refute the finding; in that case the dismissal MUST be documented with reproducible verification commands (per the example above).

### CI required-checks status post-merge

| Check | Status | Time | Notes |
|---|---|---|---|
| `Verify trust-root-modifying commits` | ✓ | < 10 s | SSH-Ed25519 signature chain validates both `826d3e2c` initial + `0563ffa7a2c27ef421232318338f0e068f2323b9` fix-commit |
| `atlas-web-playwright` | ✓ | 4m31s | Triggered via `.handoff/v2-beta-welle-18c-plan.md` doc-touch per Lesson #11 |
| `atlas-mem0g-smoke` | ✓ | 1m03s | Live HuggingFace model download + SHA-verify all 6 pins + fastembed init + 384-dim embedding output — **first end-to-end Layer-3-operational validation against live infra**. 3rd consecutive stable mem0g-smoke run (W18b initial + W18c-A + W18c-B); **promotion-to-required-check candidate threshold now MET** (operator decision, not Phase 14.7 scope) |
| `atlas-arcadedb-smoke` | (not triggered) | — | Path-filter scope |
| Aikido | skipped | — | Standing skip per repo policy |

579 tests / 0 failed / 7 ignored workspace-wide (delta +2 vs pre-Phase-B 577: 2 new `read_and_verify` match/mismatch behavioural tests). `cargo clippy --workspace --no-deps -- -D warnings` zero workspace-wide + `cargo check -p atlas-mem0g --features lancedb-backend` clean (feature-on contract preserved).

### What's next (W18c Phase C + Phase D + counsel kickoff)

1. **W18c Phase C (V1-V4 verification gap CI matrix)** — engineering-pipeline parallel-track. Cross-platform CI matrix Linux + Windows + macOS for fastembed-rs determinism + Lance v2.2 `_deletion_files` semantics + LanceDB Windows `cleanup_old_versions` behaviour. ~1 session agent-only. Closes spike §12 V1-V4 verification gaps.
2. **W18c Phase D (LanceDB ANN/search body fill-in)** — engineering-pipeline parallel-track. Replaces `Mem0gError::Backend("not yet wired")` placeholders in `crates/atlas-mem0g/src/lancedb_backend.rs::{upsert, search, erase, rebuild}` with `tokio::task::spawn_blocking`-wrapped LanceDB calls per spike §7 (NEVER `Handle::current().block_on()` — deadlocks under single-threaded scheduler). `RESUME(spawn_blocking)` markers already placed at body sites. ~1-2 sessions agent-only. **This is the welle that pivots `/api/atlas/semantic-search` to real hits** (replaces 501 stub).
3. **`atlas-mem0g-smoke` promotion-to-required-check** — queued cosmetic-eligible operator-decision. 3-stable-run threshold now met (W18b initial + W18c-A + W18c-B). Promotion adds `mem0g-smoke` to the required-check list for branch protection. NOT in Phase 14.7 scope; flag for next welle operator-runbook touch.
4. **Counsel-engagement firm selection + outreach kickoff** — parallel-track Nelson-led per `DECISION-COUNSEL-1`. With v2.0.0-beta.1 LIVE end-to-end AND Layer 3 embedder OPERATIONAL, the counsel kickoff is now MORE valuable than at any prior Atlas milestone — concrete artefact for counsel/investors/customer-prospects.

---

## 0z7. V2-β Phase 14 (W19 v2.0.0-beta.1 ship convergence) SHIPPED — 2026-05-15

> **W19 SHIPPED narrative.** §0-NEXT above is the actionable next-session entry point (now W18c-Phase-C-or-D-focused post-Phase-B-SHIPPED + Layer-3-embedder-OPERATIONAL; see §0z8 for the immediately-preceding W18c Phase B fastembed-wiring SHIPPED narrative); read this section if you need the W19 ship historical detail (what landed in PR #102 + PR #103, reviewer-dispatch outcomes, post-merge ship cascade, multi-perspective pre-tag review, fix-commit `1789e58` on `8287b97`). Phase 14 W19 closes the V2-β-1 ship convergence milestone; W18c Phase B has now SHIPPED (PR #105, §0z8), Phase C + Phase D remain queued as engineering-pipeline parallel-track to v2.0.0-beta.1 LIVE.
