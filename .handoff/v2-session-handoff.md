...
## 0-NEXT. 2026-05-15+-after-Phase-14.7 next-session entry — W18c Phase C OR Phase D (post-Phase-B-SHIPPED + Layer 3 embedder OPERATIONAL)

> **Read this section first when resuming Atlas work after 2026-05-15.** Brings a fresh agent from cold context to actionable W18c Phase C or Phase D engineering work in <10 min. v2.0.0-beta.1 is LIVE end-to-end (signed tag, GitHub Release, npm registry, Sigstore Build L3 provenance attached). **W18c Phase B SHIPPED 2026-05-15 (PR #105 `76046be`) — Layer 3 embedder is now OPERATIONAL** (`AtlasEmbedder::new` returns `Ok(_)` against real bge-small-en-v1.5 model dir; `embed()` returns 384-dim FP32 vector). `/api/atlas/semantic-search` Read-API STILL returns 501 stub — Phase D LanceDB ANN/search body fill-in is the next gate that activates real hits. Phase C closes V1-V4 cross-platform verification gaps in parallel.
>
> **🎯 RECOMMENDED PICKUP:** **`.handoff/v2-beta-welle-18c-plan.md`** Phase C section OR Phase D section (master-resident; both engineering-pipeline parallel-trackable post-Phase-B-SHIPPED). Phase C = cross-platform CI matrix Linux + Windows + macOS for fastembed-rs determinism + Lance v2.2 `_deletion_files` semantics + LanceDB Windows `cleanup_old_versions` behaviour. Phase D = `tokio::task::spawn_blocking`-wrapped LanceDB ANN/search calls per spike §7 (NEVER `Handle::current().block_on()` — deadlocks). Phase D is the one that activates `/api/atlas/semantic-search` real hits.

### 5-min snapshot

- **Master HEAD:** `76046be` (PR #105 W18c Phase B fastembed wiring merged; THIS Phase 14.7 consolidation PR's merge-commit follows). Branch protections active; admin-merge pre-authorised in `.claude/settings.local.json`.
- **V2 status:** **v2.0.0-beta.1 LIVE on master + GitHub + npm + Layer 3 embedder OPERATIONAL.** V2-β Phase 0–14.7 all SHIPPED + **v2.0.0-beta.1 LIVE end-to-end** (signed tag SHA `81d363e58eb9ec6b5234d1f4c4c091683e754a17`, Good Ed25519 `nelson@ultranova.io` fingerprint `SHA256:qq/VVJYpsgEdeQSLqU0QS/gKn6ohXJHio+VkzVX+4Zg`; GitHub Release prerelease https://github.com/ThePyth0nKid/atlas/releases/tag/v2.0.0-beta.1; npm registry `@atlas-trust/verify-wasm@2.0.0-beta.1` LIVE with dist-tags `latest = 2.0.0-beta.1` + `node = 2.0.0-beta.1`; Sigstore Build L3 provenance attached, shasum `60d9160d43e3e4de89a236b40ee584522b020c56`; wasm-publish.yml W11 race-fix validated 2nd time end-to-end). **Layer 2 ArcadeDB operational** (W17a trait + W17b driver + W17c CI/bench). **Layer 3 embedder OPERATIONAL** (Phase B SHIPPED — `fastembed::TextEmbedding::try_new_from_user_defined` wired against 11 compile-in supply-chain pins — 6 hash digests (1 × SHA-1 + 5 × SHA-256) + 5 URLs — at HuggingFace `BAAI/bge-small-en-v1.5` @ `5c38ec7c405ec4b44b94cc5a9bb96e735b38267a`; `AtlasEmbedder::new` returns `Ok(_)` against real model dir; `embed()` returns 384-dim FP32 vector; `Mutex<TextEmbedding>` interior mutability wrapper + `Pooling::Cls` per BGE upstream default; TOCTOU eliminated via `read_and_verify` single-buffer primitive). **`/api/atlas/semantic-search` Read-API STILL returns 501 stub** pending W18c Phase D LanceDB body fill-in. 31 decisions logged in `.handoff/decisions.md`. 12 ADRs total; ADR-Atlas-012 SHIPPED, ADR-Atlas-013 reserved for W18c Phase C/D implementation amendments.
- **CI required-checks for any new PR:** (1) `Verify trust-root-modifying commits` — SSH-Ed25519 enforced; (2) `atlas-web-playwright` — path-filter auto-triggers on `.handoff/**` + `.github/workflows/**` + `apps/atlas-web/**` + `crates/atlas-{signer,trust-core,verify-wasm}/**` + `packages/*/**`. **Crates-only PRs need a `.handoff/**` doc-touch to trigger playwright** (Atlas Lesson #11). `atlas-arcadedb-smoke` + `atlas-mem0g-smoke` workflows are NOT required-checks yet (promote when ≥3 unrelated PR runs stable — **`atlas-mem0g-smoke` threshold now MET as of Phase B**: W18b initial + W18c-A + W18c-B = 3 consecutive stable; promotion-to-required-check is a queued cosmetic-eligible operator-decision flagged for next welle, NOT in Phase 14.7 scope). All required CI checks GREEN on PR #105 — `Verify trust-root` ✓, `atlas-web-playwright` ✓ (4m31s), `atlas-mem0g-smoke` ✓ (1m03s — live HF model download + SHA-verify + fastembed init + 384-dim embedding). Aikido skipping.
- **Active welle:** **W18c Phase C (V1-V4 verification gap CI matrix)** OR **W18c Phase D (LanceDB ANN/search body fill-in)** — both engineering-pipeline parallel-trackable post-Phase-B-SHIPPED; Phase D activates `/api/atlas/semantic-search` real hits. Layer 3 embedder is now OPERATIONAL — the storage+retrieval layer is the only remaining pre-full-operational gate.
- **Phase A + Phase B both SHIPPED (W18c engineering-pipeline progress):** 11 supply-chain pins (1 × SHA-1 + 5 × SHA-256 + 5 × URL) compile-in and verified at every cold start via the NEW `read_and_verify` single-buffer primitive. Phase B atomically extended Phase A's 9-pin set with the 4th tokenizer pin `TOKENIZER_CONFIG_JSON_SHA256 = 9261e7d79b44c8195c1cada2b453e55b00aeb81e907a6664974b4d7776172ab3` + matching URL (discovered during Step 0 API verification when fastembed 5.13.4's `TokenizerFiles` struct surfaced `tokenizer_config_file` as a 4th required field). Phase B fix-commit extended `tools/w18c-phase-a-resolve.sh` to iterate 4 tokenizer files (not 3) for future-rotation auditable-provenance. Remaining W18c Phase C-D items are ALL agent-only: (c) **close V1-V4 verification gaps** from spike §12 (cross-platform CI matrix Linux + Windows + macOS + Lance v2.2 `_deletion_files` + model size measurement); (d) **lift LanceDB ANN/search body stubs** — currently `Mem0gError::Backend("not yet wired")`. ADR-Atlas-013 still reserved.
- **Blocked-on-Nelson (parallel, not engineering-pipeline-blocking):** Counsel-engagement firm selection + outreach (`.handoff/v2-counsel-engagement-scope.md` is RFP-ready; the `embedding_erased` audit-event payload is in counsel's review scope per ADR-Atlas-012 §5.4; **counsel-track now MORE valuable post-v2.0.0-beta.1-LIVE + Layer-3-embedder-OPERATIONAL** — concrete artefact for counsel/investors/customer-prospects with the embedder now demonstrably end-to-end functional); `RULESET_VERIFY_TOKEN` PAT configuration per `docs/OPERATOR-RUNBOOK.md` §16 (cosmetic-only, does not block merges); **`atlas-mem0g-smoke` promotion-to-required-check candidate** (3-stable-run threshold met as of Phase B; cosmetic-eligible operator decision; flagged for next welle, NOT Phase 14.7 scope). **W18c Phase A + Phase B engineering complete** — no remaining Nelson-blocked engineering items in the immediate W18c pipeline; Phase C + Phase D are agent-only.
