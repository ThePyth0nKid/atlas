...
> **Status:** v1 (Phase 3 synthesis complete, 2026-05-12). **Source documents:** Phase 1 (5 Foundation Docs) + Phase 2 (6 Multi-Angle Critiques). **Decisions:** see `.handoff/decisions.log`. **Next phase:** Phase 4 = `docs/V2-MASTER-PLAN.md` + `docs/WORKING-METHODOLOGY.md` lands on master.
>
...
**Verbatim §2:** logging level *"appropriate to the intended purpose of the system."*
...
| **Art. 50** GPAI transparency, synthetic content marking | machine-readable mark on synthetic content | `author_did = agent:*` in events.jsonl IS this machine-readable signal | **Missed-opportunity in Phase 1 — now claimed** |
...
- **Documented in Phase 3 decisions.log as DECISION-COUNSEL-1.**
...
> A consumer of Atlas at any layer can always (a) drop to Layer 1 alone, (b) run the offline WASM verifier against `events.jsonl` + `pubkey-bundle.json`, and (c) produce a deterministic ✓ VALID / ✗ INVALID answer independent of Atlas operator, Layer-2 corruption, or Layer-3 cache state. **This is the bedrock V1 property and V2 must never undermine it.**
...
- **`signed_at_rekor_inclusion_time` Δ-flagging** — events whose `event.timestamp` is more than Δ before their Rekor anchor inclusion time are flagged as suspect-backdated.
...
1. **Enrolment** — Regulator generates witness keypair on regulator-controlled HSM. Public key handed out-of-band to Atlas operator. **Phase 2 Security C-2 fix: enrolment requires M-of-N threshold-signed bundle update (NOT single operator V1.18 path) + emits `federation_enrolment_event` into Layer 1 so addition is part of verifiable record.**
...
| COSE Algorithm enum (post-quantum migration hook) | `crates/atlas-trust-core/src/cose.rs::Algorithm` (`#[non_exhaustive]`) |
...
All Phase 3 ACCEPT / MODIFY / REJECT / DEFER decisions are documented in `.handoff/decisions.log`. Each entry includes: topic, crit source, recommendation, decision, rationale, reversibility (HIGH/MED/LOW), review-after trigger. Target: ≥10 entries; actual count: see decisions.log.
