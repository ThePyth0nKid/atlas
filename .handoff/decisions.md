...
## 2026-05-15: Welle 19 — v2.0.0-beta.1 ship convergence + post-tag polish [DECISION-ARCH-W19]
- **Source:** V2-β Welle 19 (PR #102 `1f5a51f` ship convergence + PR #103 `81d363e` post-merge polish). Initial commit `8287b97` + reviewer fix-commit `1789e58` (PR #102 squash-merged); polish commit (PR #103 squash-merged) post-tag-push but pre-publication. Parallel `code-reviewer` + `security-reviewer` dispatch per Atlas Standing Protocol Lesson #8 on both PRs. Multi-perspective pre-tag review per NEW Atlas Standing Protocol Lesson #19 (architect + security + compliance/counsel + consistency/drift; all 4 GO with M-1 + M-2 + L-2 polish applied as PR #103 BEFORE tag-push). Crit source: handoff §0-NEXT W19 framing post-Phase-14.5 + plan-doc `.handoff/v2-beta-welle-19-plan.md` master-resident from Phase 13.6 (PR #99) + W18c Phase A supply-chain pins lifted (PR #100) prerequisite met.
- **Master Plan affected:** §6 Welle Decomposition (Phase 14 W18c-A + W19 BOTH SHIPPED; "Next welle" pivots from "W19 ship convergence primary OR W18c-B alternate" to "W18c Phase B fastembed wiring as primary engineering-pipeline path post-v2.0.0-beta.1-LIVE"), handoff §0z7 W19 SHIPPED narrative, `V2-BETA-ORCHESTRATION-PLAN.md` (Phase 14 + Phase 14.6 SHIPPED status flips), `V2-BETA-DEPENDENCY-GRAPH.md` (W19 SHIPPED terminus + Phase 14.6 SHIPPED node + W18c Phase B as next-target).
- **Decision:** **ACCEPT.** Workspace version bump from `2.0.0-alpha.2` to `2.0.0-beta.1` across **6 lockstep manifests** (Cargo.toml workspace + root `package.json` + 4 sub-manifests: atlas-bridge + atlas-cypher-validator + atlas-mcp-server + atlas-web; root manifest added per reviewer HIGH-1 fix) + Cargo.lock auto-regenerated + CHANGELOG `[Unreleased]` → `[2.0.0-beta.1] — 2026-05-15` conversion with V2-β tripod ship summary paragraph + new empty `[Unreleased]` above + NEW `docs/V2-BETA-1-RELEASE-NOTES.md` (~145 lines, 13 sections) + NEW `docs/SEMVER-AUDIT-V2.0-beta.md` (~135 lines, 8 sections, mirrors V1.0 methodology) + README badge alt-text update (M-2 polish via PR #103) + MCP SDK introspection bump per M-1. ~411 net-additions on PR #102 + 6 net-additions on PR #103. 577 tests pass workspace-wide; clippy `-D warnings` zero warnings; byte-pin `8962c1681a44f9569f78c5917f568c5a027ac69f727f23ba5e8f871e5e013ac4` reproduces unchanged through both InMemoryBackend AND ArcadeDbBackend.
- **Rationale:** per `DECISION-COMPLIANCE-3` / `DECISION-COUNSEL-1`, the v2.0.0-beta.1 tag itself is **internal engineering milestone** (NOT V2-β public material; counsel sign-off blocks public materials per `DECISION-COUNSEL-1` but not the tag). V2-β-1 LIVE provides **external signal** for counsel/investors/customer-prospects — concrete artefact to point at. W11 wasm-publish.yml race-fix validated **2nd time end-to-end** through this ship (first validation was V2-α-α.2 from W11; W19 is 2nd; race-fix design now has 2 independent live-registry success runs). Layer 3 SCAFFOLD-SHIPPED with W18c Phase A pins lifted, Phase B-D engineering-pipeline parallel-track to v2.0.0-beta.1 LIVE; scaffold-honesty LOUDLY stated in release notes ("`/api/atlas/semantic-search` returns 501 stub until W18c Phase B activates LanceDB ANN bodies"). Tag-immutability contract per V1.17 Welle B preserved.
- **Ship scope (lockstep-version 6 manifests):**
  - `Cargo.toml` (workspace) — line 37 version bump
  - `package.json` (root workspace-monorepo manifest) — HIGH-1 reviewer fix
  - `packages/atlas-bridge/package.json` — `@atlas/bridge`
  - `packages/atlas-cypher-validator/package.json` — `@atlas/cypher-validator`
  - `apps/atlas-mcp-server/package.json` — `atlas-mcp-server`
  - `apps/atlas-web/package.json` — `atlas-web`
- **Reviewer-dispatch outcome (PR #102 ship convergence):** parallel `code-reviewer` + `security-reviewer`. **0 CRITICAL + 1 HIGH (root package.json lockstep) + 3 MEDIUM + 1 LOW.** All applied in-commit per Lesson #3 (fix-commit `1789e58` on top of initial `8287b97`). **HIGH (overlap):** initial commit followed plan-doc's exact in-scope list (5 manifests) but root `package.json` also carried `"version": "2.0.0-alpha.2"` and would have been left stale; reviewer caught the lockstep inconsistency; fix-commit added root to in-scope. **MEDIUM closes:** (M-1) MCP server SDK introspection version string bumped to match new package version; (M-2) `docs/SEMVER-AUDIT-V2.0-beta.md` draft-artifact warning header added (the SEMVER-AUDIT companion is a new doc-pattern; explicit "draft" header reduces accidental authoritative-treatment); (M-3) cargo SemVer prerelease ordering note clarified in release notes upgrade guide (`2.0.0-beta.1 < 2.0.0` per SemVer §11). **LOW close:** CHANGELOG self-reference cleanup.
- **Reviewer-dispatch outcome (PR #103 post-merge polish):** parallel `code-reviewer` + `security-reviewer` post-tag-push pre-publication. **6/6 PASS APPROVE both reviewers.** M-1 Release Notes GDPR phrasing (compliance-perspective finding: removes "GDPR-alignment-as-property" assertion from `embedding_erased` bullet; adds `DECISION-COUNSEL-1` + W18c Phase D pending qualifiers) + M-2 README badge alt-text (accessibility-perspective finding: "Article 12" → "Art. 12 — design-aligned") + L-2 Phase D parenthetical (consistency-perspective finding; combined with M-1) — all 3 flagged in the multi-perspective pre-tag review and landed atomically as PR #103 BEFORE tag-push.
- **Multi-perspective pre-tag review (4 perspectives, all GO per NEW Atlas Standing Protocol Lesson #19):** before pushing the signed tag (irreversible-public-action step), parent dispatched 4 parallel reviewers (architect + security + compliance/counsel + consistency/drift). All 4 returned **GO**. M-1 + M-2 + L-2 polish landed as PR #103 before tag-push per standing directive "Geh mit A, beste Sicherheit + Codequalität". Pattern now codified as Lesson #19 in handoff §0-NEXT.
- **Post-merge ship cascade outcome (parent-led, documented for traceability):**
...
- **Architectural posture preserved:** trust property unchanged (Layer 1 invariants intact); byte-pin reproduces; Layer 3 NEVER trust-authoritative; every `SemanticHit.event_uuid` cite-back trust intact (when Layer 3 activates in Phase B). Tag-immutability contract per V1.17 Welle B preserved (signed tags permanent; future amendments via forward-version SemVer-patch v2.0.0-beta.2 etc).
- **CI status post-merge:** all required CI checks green on PR #102 + PR #103: `Verify trust-root-modifying commits` ✓ (SSH-Ed25519), `atlas-web-playwright` ✓ (triggered via `apps/atlas-web/package.json` version bump + `.handoff/**` touch per Lesson #11), `atlas-mem0g-smoke` ✓ (NEW W18b CI workflow; first post-v2.0.0-beta.1 validation). `wasm-publish.yml` auto-triggered post-tag-push and completed green end-to-end.
- **SemVer impact:** V2-β-1 surface contract documented in NEW `docs/SEMVER-AUDIT-V2.0-beta.md` companion (cleaner separation from V1.0 baseline contract; V1.0 §1-§9 untouched). Locked surfaces: atlas-mem0g `SemanticCacheBackend` trait + `SemanticHit` + `Mem0gError` (`#[non_exhaustive]`) + `InvalidationPolicy`. Internal-deferred-to-V2-γ-stability: `AtlasEmbedder` (still fail-closed pending Phase B). Locked-Behind-Flag: `LanceDbCacheBackend` (`lancedb-backend` feature default-OFF).
- **Confidence:** HIGH on ship convergence event (tag pushed, npm publish green end-to-end, Sigstore provenance verified, all required CI checks green); HIGH on scaffold-honesty disclosure (release notes + SEMVER-AUDIT-V2.0-beta + CHANGELOG summary paragraph + Read-API 501 response all carry consistent scaffold-status messaging); HIGH on tag-immutability preservation (per V1.17 Welle B precedent); HIGH on W11 wasm-publish.yml race-fix design (2 independent live-registry success runs); MEDIUM on customer-perception risk (R-W19-5 — scaffold-ship signals "ready" — mitigated by 4-channel consistent messaging + counsel-track gating per `DECISION-COUNSEL-1` blocking public marketing).
- **Reversibility:** LOW. Tag-immutability per V1.17 Welle B (signed tags permanent). npm publish has 72h unpublish window but unpublish is industry-bad-practice — would damage Atlas's supply-chain reputation. Forward-version SemVer-patch (v2.0.0-beta.2) is the recovery path for any post-ship amendment, e.g. if W18c Phase B activation surfaces a fastembed API surface incompatibility requiring breaking change in atlas-mem0g.
- **Open question (OQ for W18c Phase B / V2-γ):** OQ-W19-1: customer-perception drift if W18c Phase B takes >1 session (release notes scaffold-status messaging may need refresh in v2.0.0-beta.2 if scaffold→operational transition has unexpected delay). OQ-W19-2: dist-tag `latest` confusion if early alpha-track users auto-upgrade and surface Layer 3 501 stub before reading release notes — mitigated by R-W19-3 precedent-aligned acceptable behaviour per V2-α-α.2 W11 PR #76. OQ-V2-γ: customer 10M-event-workspace deployment-telemetry validation post-W18c Phase B activation (T2 trigger from ADR-010 §4.10).
- **Review-after:** W18c Phase B fastembed wiring ship event (next forward-version tag would be v2.0.0-beta.2 if Phase B surfaces design amendment per ADR-Atlas-013 reservation). First-customer 10M-event-workspace deployment-telemetry validation post-W18c Phase B (T2 trigger from ADR-010 §4.10).

---

---

## 2026-05-15: Welle 18c Phase B — fastembed try_new_from_user_defined wiring + supply-chain pin set 9→11 + TOCTOU-free read_and_verify [DECISION-ARCH-W18c-B]
- **Source:** V2-β Welle 18c Phase B (PR #105, master commit `76046be`). Initial commit `826d3e2c` SSH-Ed25519 signed by Phase B subagent + reviewer fix-commit `0563ffa7a2c27ef421232318338f0e068f2323b9` on top per Atlas Standing Protocol Lesson #3 (squash-merged). Parallel `code-reviewer` + `security-reviewer` dispatch per Lesson #8. **Multi-perspective pre-tag review (Lesson #19) did NOT apply** — Phase B is engineering-pipeline (no irreversible-public-action like signed-tag push or npm publish); standard 2-reviewer dispatch was the correct posture. Source documents: handoff §0-NEXT W18c framing (post-Phase-14.6) + plan-doc `.handoff/v2-beta-welle-18c-plan.md` Phase B section (master-resident since Phase 13.6) + Phase A SHIPPED state (PR #100 master commit `28700ae`) providing the 9 compile-in pins prerequisite.
- **Master Plan affected:** §6 Welle Decomposition (Phase 14 W18c-A + W19 + W18c-B all SHIPPED; "Next welle" pivots from "W18c Phase B as primary engineering-pipeline" to "W18c Phase C (V1-V4 verification gap CI matrix) OR W18c Phase D (LanceDB ANN/search body fill-in)" — both engineering-pipeline-parallel-trackable; Phase D activates `/api/atlas/semantic-search` real hits), handoff §0z8 W18c Phase B SHIPPED narrative, `V2-BETA-ORCHESTRATION-PLAN.md` (Phase 14 + Phase 14.7 status flips), `V2-BETA-DEPENDENCY-GRAPH.md` (W18c-B SHIPPED node + Phase 14.7 SHIPPED node + W18c-C/D next-target downstream).
- **Decision:** **ACCEPT.** Phase B's `read_and_verify` primitive + `supply_chain.rs` module split + 4th tokenizer pin atomic extension; preserve W18b HIGH-2 fix (`try_new_from_user_defined` exclusive path, no `Default::default()` bypass); preserve byte-pin invariant `8962c1681a44…e013ac4` reproducing through both InMemoryBackend AND ArcadeDbBackend; preserve `SemanticCacheBackend` + `SemanticHit` + `Mem0gError` SemVer trait surface. 7 files changed across PR #105; +1046/-404 lines; NEW `crates/atlas-mem0g/src/supply_chain.rs` (~552 LOC extracted module). 579 tests pass workspace-wide / 0 failed / 7 ignored (delta +2 vs pre-Phase-B 577: 2 new `read_and_verify` match/mismatch behavioural tests in `supply_chain.rs`); clippy `-D warnings` zero workspace-wide; `cargo check -p atlas-mem0g --features lancedb-backend` clean.
- **Phase B implementation posture:** Layer 3 embedder now **OPERATIONAL** (`AtlasEmbedder::new` returns `Ok(_)` against real bge-small-en-v1.5 model dir; `embed()` returns 384-dim FP32 vector; verified end-to-end against live HuggingFace via `atlas-mem0g-smoke` CI in 1m03s). **BUT `/api/atlas/semantic-search` Read-API STILL returns 501 stub** — Phase D (LanceDB ANN/search body fill-in) is separate scope. Phase B activates the embedder; Phase D activates the storage+retrieval. The full Layer 3 operational mode arrives with Phase D.
- **Critical Phase B discovery — atomic pin set extension 9 → 11:** during Step 0 fastembed-rs 5.13.4 API verification (`cargo doc -p fastembed --features lancedb-backend`), the `fastembed::TokenizerFiles` struct surfaced FOUR required fields (Phase A's research-stage understanding was three): `tokenizer_file` + `config_file` + `special_tokens_map_file` + **`tokenizer_config_file`** (the 4th). Phase B atomically extended the pin set with `TOKENIZER_CONFIG_JSON_SHA256 = 9261e7d79b44c8195c1cada2b453e55b00aeb81e907a6664974b4d7776172ab3` + matching `TOKENIZER_CONFIG_JSON_URL` (revision-pinned to `5c38ec7c…`), resolved live via `curl -sL ... | sha256sum`. Total pin count: 9 (5 SHA + 4 URL) → **11** (6 SHA + 5 URL = 1 × SHA-1 + 5 × SHA-256 + 5 × URL). `_STRUCTURAL_PIN_CHECK` const-eval + `pins_well_formed_after_lift` + `pins_are_non_empty` + integration test `pins_remain_well_formed_post_phase_a_lift` ALL atomically extended. Phase B fix-commit (security-reviewer MEDIUM-2) extended `tools/w18c-phase-a-resolve.sh` to iterate 4 tokenizer files (not 3) — single-script auditable-provenance chain restored for future revision rotations. Atomic-lift contract preserved.
- **Architectural choices accepted (Phase B):** (a) **`Mutex<TextEmbedding>` interior-mutability wrapper** — fastembed `embed(&mut self)` requires exclusive access; Atlas's `AtlasEmbedder` is `Arc<...>`-shared across the dispatch + Read-API code paths (immutable `&self` callable); panic-on-poison acceptable per atlas-mem0g single-process posture. (b) **`Pooling::Cls`** per BGE-Small-EN-v1.5 upstream default (matches `fastembed::get_default_pooling_method(BGESmallENV15)` exactly + spike §3.4 reference; preserves embedding-shape compatibility with any future fastembed version surfacing alternative pooling modes). (c) **`supply_chain.rs` module split** — Phase A's `embedder.rs` mixed embedder-init code with pin-verification code (~970 LOC combined); Phase B extracted verification path to NEW `supply_chain.rs` (~552 LOC). Clean cargo + reviewer + test boundary; future Phase D LanceDB body fill-in does not touch supply-chain logic. Re-exports preserved at `crates/atlas-mem0g/src/lib.rs` so external API surface is unchanged.
- **TOCTOU eliminated via `read_and_verify` single-buffer primitive (security-reviewer HIGH-1 closure):** original implementation streamed model file through `sha256_file()` (read pass 1) then later opened it for fastembed init (read pass 2). Between pass 1 and pass 2 an attacker with FS write access (Atlas's threat model) could swap bytes — verify-then-use TOCTOU race. Fix-commit introduced `read_and_verify(path, expected_sha) -> Mem0gResult<Vec<u8>>` primitive that performs a single contiguous `Vec<u8>` read AND hashes that exact buffer; the returned buffer (already SHA-verified) is the EXACT bytes fed to fastembed via `TokenizerFiles { tokenizer_file: buf, ... }`. Bytes hashed ARE bytes consumed — single-buffer invariant eliminates the verify-then-use window. Applied at all 4 verify-then-use call-sites (`model.onnx` + 4 tokenizer files via 4 SHA-256 pins).
- **Reviewer-dispatch outcome:** parallel `code-reviewer` + `security-reviewer` (Atlas Standing Protocol Lesson #8). **code-reviewer:** REJECT (APPROVE-WITH-FIXES) — 0 CRITICAL / **2 HIGH** (one **DISMISSED** via behavioural-test settlement per Lesson #2: claim that `TOKENIZER_CONFIG_JSON_SHA256` was 63 chars verified WRONG via parent `echo -n '<value>' | wc -c` = 64 + independent HF `curl|sha256sum` re-resolve byte-identical match; one applied: missing reqwest timeout, `Client::builder().timeout(Duration::from_secs(300))` 5-min ceiling per ADR-012 cold-start budget) / **4 MEDIUM** (all applied: M-1 `Mutex<TextEmbedding>` poison-recovery doc, M-2 `Pooling::Cls` choice rationale + citation, M-3 `supply_chain.rs` module-level rustdoc with pin-set invariant table, M-4 `embed()` error-path coverage disambiguation) / **3 LOW** (1 applied; 2 deferred to Phase C/D + operator-runbook welle). **security-reviewer:** APPROVE-WITH-FIXES — 0 CRITICAL / **1 HIGH TOCTOU verify-then-use** (applied via `read_and_verify` primitive above) / **2 MEDIUM** (both applied: M-1 reqwest TLS `https_only(true)` re-asserted in NEW `supply_chain` module — was lost in module split; M-2 `tools/w18c-phase-a-resolve.sh` extension to iterate 4 tokenizer files for future-rotation auditable-provenance) / **3 LOW** (deferred to Phase C/D + operator-runbook welle per Lesson #3 V2-γ-scope clean-up exception). All HIGH + applied-MEDIUM + applied-LOW in fix-commit `0563ffa7a2c27ef421232318338f0e068f2323b9` on top of initial `826d3e2c`.
- **Behavioural-test settlement note (code-reviewer HIGH-1 dismissal per Lesson #2):** the dismissed code-reviewer HIGH-1 (claim: pin is 63 chars) was refuted via three independent signals: (1) direct character count `echo -n '<value>' | wc -c` = 64; (2) independent HuggingFace re-resolve `curl -sL .../tokenizer_config.json | sha256sum` byte-identical to compile-in value; (3) in-tree behavioural test `pins_remain_well_formed_post_phase_a_lift` asserts `len() == 64` and passed unmodified. Reviewer-finding dismissal is documented inline in the fix-commit message + handoff §0z8 narrative + here. Pattern (extending Lesson #2): reviewer findings are non-optional EXCEPT when behavioural tests + independent verification authoritatively refute the finding; in that case the dismissal MUST be documented with reproducible verification commands.
- **Posture preserved through Phase B ship:** (a) **W18b HIGH-2 fix preserved** — `try_new_from_user_defined` exclusive init path; NO `Default::default()` bypass anywhere; grepped clean by security-reviewer. (b) **Trait surface SemVer-locked** — `SemanticCacheBackend` + `SemanticHit` + `Mem0gError` (`#[non_exhaustive]`) + `InvalidationPolicy` shapes unchanged from W18b. (c) **Byte-pin reproduces** `8962c1681a44…e013ac4` unchanged through both InMemoryBackend AND ArcadeDbBackend post-Phase-B. (d) **Embeddings OUTSIDE canonicalisation pipeline** — lib.rs invariant #3 honoured; Phase B touches NOT a single canonical-byte code path. (e) **Layer 3 NEVER trust-authoritative** — every `SemanticHit.event_uuid` is structurally non-optional `String`; cite-back trust property end-to-end intact. (f) **Hermes-skill `npx` distribution intact** — pure-Rust embedder boundary preserved (no Python, no second JVM). (g) **Fail-closed posture for any future drift** — if any of the 11 pins drifts at runtime (model file replaced on disk, network swap, etc.), `read_and_verify` returns `Mem0gError::SupplyChainMismatch` and `AtlasEmbedder::new` returns the error — never proceeds with unverified bytes.
- **Supply-chain pin contract:** **11 total pins compile-in** = 1 × SHA-1 revision (`HF_REVISION_SHA = 5c38ec7c405ec4b44b94cc5a9bb96e735b38267a`) + 5 × SHA-256 file digests (`ONNX_SHA256` + `TOKENIZER_JSON_SHA256` + `CONFIG_JSON_SHA256` + `SPECIAL_TOKENS_MAP_SHA256` + NEW `TOKENIZER_CONFIG_JSON_SHA256 = 9261e7d79b44c8195c1cada2b453e55b00aeb81e907a6664974b4d7776172ab3`) + 5 × URL (model.onnx + 4 tokenizer files; revision-SHA-in-path invariant tested across all 5). Atomic-lift contract preserved — any future revision rotation runs `tools/w18c-phase-a-resolve.sh` once and surfaces all 6 hashes uniformly. The structural-pin const-eval block fails compilation on accidental blanking; the `pins_well_formed_after_lift` integration test fails on any drift in length / lowercase-hex / huggingface.co origin / revision-SHA-in-path.
- **CI status post-merge:** all required CI checks green on PR #105: `Verify trust-root-modifying commits` ✓ (< 10 s; SSH-Ed25519 chain validates both `826d3e2c` initial + `0563ffa7a` fix-commit); `atlas-web-playwright` ✓ (4m31s; triggered via `.handoff/v2-beta-welle-18c-plan.md` doc-touch per Lesson #11); `atlas-mem0g-smoke` ✓ (1m03s; live HuggingFace model download + SHA-verify all 6 pins + fastembed init + 384-dim embedding output — **first end-to-end Layer-3-operational validation against live infra**). `atlas-arcadedb-smoke` + HSM workflows not triggered (path-filter scope). Aikido skipping. **`atlas-mem0g-smoke` is now a promotion-to-required-check candidate** — 3rd consecutive stable run (W18b initial + W18c-A + W18c-B); threshold MET per handoff §0-NEXT promotion criteria. Promotion is a separate operator-decision (NOT in Phase 14.7 scope; flagged as queued option for next welle).
- **SemVer impact:** SemVer-additive within `atlas-mem0g` 0.1.0. NEW `pub const TOKENIZER_CONFIG_JSON_SHA256` + `TOKENIZER_CONFIG_JSON_URL` items. NEW `pub mod supply_chain` re-exports (existing `pub use` paths preserved). `Mem0gError::SupplyChainMismatch` enum variant unchanged. `SemanticCacheBackend` + `SemanticHit` + `InvalidationPolicy` trait surface unchanged. `AtlasEmbedder` Internal-deferred-to-V2-γ-stability per `docs/SEMVER-AUDIT-V2.0-beta.md` — Phase B operational activation does NOT promote `AtlasEmbedder` to Locked status (still Internal until V2-γ). No new event-kinds. No Read-API surface change (501 stub posture preserved pending Phase D). v2.0.0-beta.1 tag remains valid; forward-version SemVer-patch (v2.0.0-beta.2) is the recovery path if Phase C or Phase D surface design amendments per ADR-Atlas-013 reservation.
- **Confidence:** HIGH on Layer 3 embedder OPERATIONAL state (verified end-to-end via live `atlas-mem0g-smoke` CI 1m03s — model downloaded, all 6 SHA-256 verified, fastembed init succeeded, 384-dim FP32 vector emitted); HIGH on TOCTOU elimination correctness (`read_and_verify` single-buffer invariant — bytes hashed ARE bytes consumed; 2 new behavioural tests in `supply_chain.rs`); HIGH on architectural choices (`Mutex<TextEmbedding>` standard Rust pattern; `Pooling::Cls` matches BGE upstream default); HIGH on byte-pin preservation + W18b HIGH-2 fix preservation + trait SemVer surface preservation (grepped clean by security-reviewer); MEDIUM-HIGH on cross-platform determinism (Phase C scope); MEDIUM on Phase D unblockedness (LanceDB API surface for `spawn_blocking`-wrapped calls is documented in spike §7 but not yet exercised end-to-end).
- **Reversibility:** MED-HIGH. Phase B code is reverted via forward-version SemVer-patch v2.0.0-beta.2 if Phase C verification surfaces design amendment. Pin set changes (9 → 11) are forward-only — once compiled-in pins lift, downgrading is operationally pointless (reverts would re-introduce the 4-tokenizer-file gap in `try_new_from_user_defined`). The `supply_chain.rs` module split is API-compatible via re-exports; future merge-back is mechanical.
- **Open question (OQ for W18c Phase C / Phase D / V2-γ):** OQ-W18c-B-1: cross-platform determinism (Phase C scope) — fastembed-rs determinism on Linux + Windows + macOS CI matrix under pinned (ORT-version, threads=1, FP32) is documented but not formally guaranteed; Phase C TDD-RED test in `crates/atlas-mem0g/tests/embedding_determinism.rs` will close. OQ-W18c-B-2: LanceDB API surface for `spawn_blocking`-wrapped calls (Phase D scope) — `RESUME(spawn_blocking)` markers already placed at body sites; `tokio::task::spawn_blocking` is the ONLY correct pattern per spike §7. OQ-V2-γ: model-rotation operator-runbook — Phase B preserved the auditable resolver helper, but the V2-γ operator-runbook needs a documented rotation procedure (semi-automated via re-running `tools/w18c-phase-a-resolve.sh` against the new revision SHA + parent reviewing the diff against the previous compile-in values).
- **Review-after:** W18c Phase D ship event (next welle that pivots `/api/atlas/semantic-search` to real hits; if Phase D surfaces fastembed-vs-LanceDB integration surprise, ADR-Atlas-013 captures the amendment). First-customer 10M-event-workspace deployment-telemetry validation post-W18c Phase D (T2 trigger from ADR-010 §4.10).
- **Reference IDs:** `DECISION-ARCH-W18c-B` (forward refs in handoff §0z8 + V2-MASTER-PLAN §6 Phase 14 W18c-B SHIPPED row + V2-BETA-ORCHESTRATION-PLAN Phase 14 W18c-B + V2-BETA-DEPENDENCY-GRAPH W18c-B SHIPPED node).

---

**End of decisions.md.** 31 decisions documented. All Phase-2 CRITICAL findings addressed (accepted/modified/deferred/rejected with rationale). V2-α Welle 1 + Welle 2 strategic decisions added 2026-05-12. V2-β Welle 17b + 17c shipped 2026-05-14 (W17a carry-overs closed in 17b; W17b regressions surfaced + closed in 17c via live CI). V2-β Welle 18 Phase A shipped 2026-05-15 (Mem0g Layer-3 cache design, 8 binding sub-decisions). V2-β Welle 18b shipped 2026-05-14 (Mem0g Layer-3 cache implementation, W17a-pattern Phase-A-scaffold). V2-β Welle 18c Phase A shipped 2026-05-15 (Mem0g supply-chain constants lifted). V2-β Welle 19 shipped 2026-05-15 — v2.0.0-beta.1 LIVE on master + GitHub + npm; signed tag `81d363e58eb9ec6b5234d1f4c4c091683e754a17` Good Ed25519; Sigstore Build L3 provenance attached; npm dist-tags `latest = 2.0.0-beta.1` + `node = 2.0.0-beta.1`; W11 wasm-publish.yml race-fix validated 2nd time end-to-end. **V2-β Welle 18c Phase B shipped 2026-05-15 — Layer 3 embedder OPERATIONAL** (PR #105 `76046be`; initial `826d3e2c` + fix-commit `0563ffa7a2c27ef421232318338f0e068f2323b9`; supply-chain pin set atomically extended 9 → 11 with NEW `TOKENIZER_CONFIG_JSON_SHA256`; TOCTOU eliminated via `read_and_verify` single-buffer primitive; `AtlasEmbedder::new` returns `Ok(_)` against real bge-small-en-v1.5 model dir; `embed()` returns 384-dim FP32 vector verified end-to-end via live `atlas-mem0g-smoke` CI 1m03s; 579 tests pass; clippy zero; byte-pin reproduces; W18b HIGH-2 fix preserved). `/api/atlas/semantic-search` Read-API STILL returns 501 stub pending W18c Phase D LanceDB body fill-in. ADR-Atlas-013 still reserved for W18c Phase C/D design amendments.
