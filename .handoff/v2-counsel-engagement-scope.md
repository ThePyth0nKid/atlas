...
**Atlas in one paragraph.** Atlas is a verifiable-knowledge-graph backend for AI agents. Every fact written by an agent is signed (Ed25519 + COSE_Sign1 over a canonical CBOR signing-input, RFC 8949 §4.2.1), hash-chained, anchored to the Sigstore Rekor public transparency log via RFC 6962 Merkle inclusion proofs, and independently verifiable via an Apache-2.0 offline WASM verifier the regulator runs in their own browser. Atlas's V1 stable-API verifier ships on npm as `@atlas-trust/verify-wasm@1.0.1` (live since 2026-05-12) — providing the load-bearing write-side trust property used by every downstream Atlas consumer. V2 adds a deterministically-rebuildable graph projection layer (Layer 2) and a fast-retrieval cache (Layer 3) without compromising the Layer-1 trust invariant; the V2 work-in-progress ships under the Atlas-meta-release tag `v2.0.0-alpha.2` (also published on npm alongside the V1 stable line for early integrators), with `v2.0.0-alpha.3` candidate consolidating on master at the time of writing.
